export type LanguageOption = {
  code: string;
  label: string;
};

export const languageOptions: LanguageOption[] = [
  { code: "en", label: "English" },
  { code: "zh-Hans", label: "简体中文" },
  { code: "zh-Hant", label: "繁體中文" },
  { code: "ko", label: "한국어" },
  { code: "ja", label: "日本語" },
  { code: "es", label: "Español" },
  { code: "fr", label: "Français" },
  { code: "de", label: "Deutsch" },
  { code: "it", label: "Italiano" },
  { code: "pt", label: "Português" },
  { code: "ru", label: "Русский" },
  { code: "th", label: "ไทย" },
  { code: "vi", label: "Tiếng Việt" },
  { code: "id", label: "Bahasa Indonesia" },
  { code: "tl", label: "Filipino" },
];

const languageLabelMap = new Map(
  languageOptions.map((option) => [option.code, option.label])
);

export type LanguageCode = (typeof languageOptions)[number]["code"];

type DeepStringify<T> = T extends string ? string : T extends object ? { [K in keyof T]: DeepStringify<T[K]> } : T;

const uiCopy = {
  en: {
    common: {
      back: "Back",
      reload: "Reload",
      translate: "Translate",
      openCamera: "Open camera",
      menu: "Menu",
      guide: "Guide",
    },
    language: {
      change: "Change language",
      select: "Select language",
      list: "Language list",
      modalTitle: "Language",
      fallback: "Language",
    },
    drawer: {
      historyLabel: "History drawer",
    },
    home: {
      bannerLabel: "Banner",
      bannerAlt: "Banner",
    },
    cta: {
      go: "Go",
      continue: "Continue",
    },
    captureHeader: {
      menu: "Menu",
      scan: "Scan",
      map: "Map",
    },
    cameraPrompt: {
      openCamera: "Open camera",
    },
    chat: {
      uploadPreview: "Upload preview",
      removeAttachment: "Remove attachment",
      tapToOpen: "Tap to open input",
      camera: "Chat camera",
      gallery: "Chat gallery",
      messageInput: "Message input",
      sendMessage: "Send message",
      send: "Send",
      sendDisabled: "Send disabled",
      attachmentGuide: "Your AI guide — Ask me anything.",
    },
    attachment: {
      cameraPhoto: "Camera photo",
      photoLibrary: "Photo library",
    },
    camera: {
      unavailable: "Camera unavailable",
      close: "Close",
      translate: "Translate",
      openGallery: "Open gallery",
      capturePhoto: "Capture photo",
      flipCamera: "Flip camera",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "Back",
      closeMenu: "Close menu",
      toggleMenu: "Toggle menu",
      nav: {
        stores: "Stores",
        groups: "Groups",
        prompt: "Prompt",
        logout: "Logout",
      },
      title: {
        stores: "Stores",
      },
    },
    browser: {
      chromeMock: "Browser chrome mock",
    },
    history: {
      sectionConversation: "Conversations",
      sectionExplore: "Explore menu",
      favorites: "Favorites",
      popular: "Popular",
      sectionStore: "Restaurant",
      storeInfo: "Restaurant info",
      sectionPreferences: "Preferences",
      preferencesAllergy: "Allergies & diet",
      exploreOther: "Explore other restaurants",
      title: "History title...",
      brandTagline: "Data infrastructure for authentic food knowledge.",
      sidebarTitle: "History",
      newChat: "New chat",
      empty: "No conversations yet",
      writeReview: "Write a review",
      staffSection: "Staff",
      staffRegisterMenu: "Add menu (photo)",
      staffAnswerQuestions: "Answer questions",
      staffCheckItems: "Review checklist",
      staffTodayMenu: "Today's menu",
      staffBulkEdit: "Fix in bulk (chat)",
      staffMenuFix: "Edit menus",
      staffProcurement: "Scan delivery slip",
      staffShareLinks: "Share link",
      staffPreviewCustomer: "Preview as customer",
    },
    hero: {
      heading: "Snap & Translate",
      sub: "Tap the camera to start",
    },
    explore: {
      title: "Explore ",
      subtitle: "Explore with ",
      placeholder: "Ask about the area...",
      searchPlaceholder: "Search restaurants",
      allCities: "All",
      nearbyTitle: "Nearby",
      listTitle: "Restaurants",
      loadMore: "Load more",
      noResults: "No restaurants found",
    },
    suggestions: {
      guide: "Your AI guide — Ask me anything.",
      chips: ["How to use it?", "What can it do?", "Dining etiquette"],
    },
    restaurant: {
      title: "Select Restaurant",
      subtitle: "Choose a restaurant to explore",
      selectPlaceholder: "Select a restaurant...",
      continue: "Continue",
      noSelection: "Please select a restaurant",
      chatPlaceholder: "Ask me anything",
      chatGuide: "AI guide for {name} — Ask me anything about this restaurant!",
      signatureDish: "What's the specialty?",
      aboutCuisine: "Tell me about {cuisine} cuisine",
      bestTime: "What do you recommend?",
      dietaryOptions: "I have dietary restrictions",
      undecided: "I can't decide",
      hearty: "Full meal",
      singleDish: "Small dish",
      lightAppetizer: "Appetizer / Salad",
      seasonalMenu: "Seasonal?",
      drinksMenu: "Drinks?",
      googleReview: "Review",
      reviewPromptText: "If you enjoyed it, a quick review helps the next visitors 🌟",
    },
    nfg: {
      ingredients: "Ingredients",
      allergens: "Allergens",
      restrictions: "Dietary Restrictions",
      flavorProfile: "Flavor Profile",
      calories: "Est. Calories",
      taxNote: "Tax",
      vadBadge: "Verified",
      pendingBadge: "Unverified",
      aiBadge: "AI Estimated",
      confidence: "Confidence",
      texture: "Texture",
      textureDrink: "Mouthfeel",
      pairing: "Pairs with",
      pairingDrink: "Pairs with (food)",
      howToEat: "How to eat",
      howToEatDrink: "How to drink",
      story: "Story",
      storyDrink: "About this drink",
      servingStyle: "Serving",
      tasteChart: "Taste",
      kidFriendly: "Kid-friendly",
      notKidFriendly: "May not suit kids",
      suggestEdit: "Suggest edit",
      abv: "ABV",
      tasteUmami: "Umami",
      tasteSweetness: "Sweet",
      tasteSourness: "Sour",
      tasteSaltiness: "Salty",
      tasteBitterness: "Bitter",
      tasteSpiciness: "Spicy",
      tasteRichness: "Rich",
      tasteLightness: "Fresh",
      tasteVolume: "Volume",
      tasteLocality: "Local",
    },
    quickExplain: {
      scanning: "Analyzing...",
      verified: "Owner-verified",
      pending: "On file",
      aiEstimate: "AI Estimate",
      newItem: "New",
      title: "Quick Explain ({n} items)",
    },
    feedback: {
      copy: "Copy",
      copied: "Copied!",
      share: "Share",
    },
    qrScanner: {
      title: "Scan QR Code",
      close: "Close",
      loading: "Starting camera...",
      cameraError: "Unable to access camera. Please check permissions.",
      invalidQR: "Invalid QR code format.",
      retry: "Retry",
      redirecting: "Redirecting to restaurant...",
      torchOn: "Light On",
      torchOff: "Light Off",
      switchCamera: "Switch",
      instructions: "Point your camera at the restaurant QR code",
    },
    capture: {
      menuAnalysis: "Menu Analysis ({n} items)",
      imageAnalyzedWith: 'Analyzed image of "{q}".',
      imageAnalyzed: "Menu image analyzed.",
      notFound: "Restaurant not found",
      checkUrl: "Please check the URL",
      loading: "Loading...",
      writeReview: "Write a review",
      submitReview: "Write a review",
      reviewPlaceholder: "Food impressions, atmosphere, recommendations...",
      submitting: "Sending...",
      submit: "Submit",
      reviewSubmitted: "Review submitted!",
      comparing: "Comparing",
      compare: "Compare",
      less: "Less",
      moreDetails: "More details",
      relatedMenu: "Related",
      tellMeMore: "Tell me more about #{num} {name}",
      openMap: "Map",
    },
    webLanding: {
      exploreMenu: "Explore Menu",
      makeReservation: "Make a Reservation",
      scanMenu: "Scan Menu",
      hours: "Hours",
      access: "Access",
      closed: "Closed",
    },
  },
  ja: {
    common: {
      back: "戻る",
      reload: "再読み込み",
      translate: "翻訳",
      openCamera: "カメラを開く",
      menu: "メニュー",
      guide: "ガイド",
    },
    language: {
      change: "言語を変更",
      select: "言語を選択",
      list: "言語一覧",
      modalTitle: "言語",
      fallback: "言語",
    },
    drawer: {
      historyLabel: "履歴ドロワー",
    },
    home: {
      bannerLabel: "バナー",
      bannerAlt: "バナー",
    },
    cta: {
      go: "進む",
      continue: "続ける",
    },
    captureHeader: {
      menu: "メニュー",
      scan: "翻訳",
      map: "地図",
    },
    cameraPrompt: {
      openCamera: "カメラを開く",
    },
    chat: {
      uploadPreview: "アップロードプレビュー",
      removeAttachment: "添付を削除",
      tapToOpen: "タップで入力を開く",
      camera: "チャットカメラ",
      gallery: "チャットギャラリー",
      messageInput: "メッセージ入力",
      sendMessage: "送信",
      send: "送信",
      sendDisabled: "送信（無効）",
      attachmentGuide: "AIガイドです。何でも聞いて下さい。",
    },
    attachment: {
      cameraPhoto: "カメラ写真",
      photoLibrary: "フォトライブラリ",
    },
    camera: {
      unavailable: "カメラを利用できません",
      close: "閉じる",
      translate: "翻訳",
      openGallery: "ギャラリーを開く",
      capturePhoto: "写真を撮影",
      flipCamera: "カメラ切替",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "戻る",
      closeMenu: "メニューを閉じる",
      toggleMenu: "メニュー切替",
      nav: {
        stores: "店舗",
        groups: "グループ",
        prompt: "プロンプト",
        logout: "ログアウト",
      },
      title: {
        stores: "店舗",
      },
    },
    browser: {
      chromeMock: "ブラウザUIモック",
    },
    history: {
      sectionConversation: "会話",
      sectionExplore: "メニュー探索",
      favorites: "お気に入り",
      popular: "人気ランキング",
      sectionStore: "店舗",
      storeInfo: "店舗情報",
      sectionPreferences: "個人設定",
      preferencesAllergy: "アレルギー・食事スタイル",
      exploreOther: "他の店舗を見る",
      title: "履歴タイトル・・・",
      brandTagline: "食を正しく伝える、データインフラ",
      sidebarTitle: "履歴",
      newChat: "新しい会話",
      empty: "まだ会話がありません",
      writeReview: "クチコミを書く",
      staffSection: "スタッフ",
      staffRegisterMenu: "メニューを登録（写真）",
      staffAnswerQuestions: "質問に答える",
      staffCheckItems: "確認事項をチェック",
      staffTodayMenu: "今日の献立",
      staffBulkEdit: "まとめて直す（チャット）",
      staffMenuFix: "メニューを修正",
      staffProcurement: "納品書を撮る",
      staffShareLinks: "リンクを共有",
      staffPreviewCustomer: "お客様画面で確認",
    },
    hero: {
      heading: "撮って翻訳",
      sub: "カメラタップでスタート",
    },
    explore: {
      title: " を探索",
      subtitle: " と一緒に探索",
      placeholder: "周辺について質問...",
      searchPlaceholder: "レストランを探す",
      allCities: "すべて",
      nearbyTitle: "近くのお店",
      listTitle: "店舗一覧",
      loadMore: "もっと見る",
      noResults: "該当するレストランが見つかりませんでした",
    },
    suggestions: {
      guide: "AIガイドです。何でも聞いて下さい。",
      chips: ["どう使うの？", "何ができるの？", "和食のマナー"],
    },
    restaurant: {
      title: "レストランを選択",
      subtitle: "探索するレストランを選んでください",
      selectPlaceholder: "レストランを選択...",
      continue: "続ける",
      noSelection: "レストランを選択してください",
      chatPlaceholder: "何でも聞いてください",
      chatGuide: "{name}のAIガイドです。このレストランについて何でも聞いてください！",
      signatureDish: "名物は？",
      aboutCuisine: "{cuisine}料理について教えてください",
      bestTime: "おすすめは？",
      dietaryOptions: "食べられないものがある",
      undecided: "迷ってる",
      hearty: "がっつり",
      singleDish: "一品料理",
      lightAppetizer: "前菜・サラダ",
      seasonalMenu: "季節メニューは？",
      drinksMenu: "ドリンクは？",
      googleReview: "クチコミ",
      reviewPromptText: "次に来る人の参考になるので、よかったらクチコミも残してください 🌟",
    },
    nfg: {
      ingredients: "主な材料",
      allergens: "アレルゲン",
      restrictions: "食事制約",
      flavorProfile: "味の特徴",
      calories: "推定カロリー",
      taxNote: "税表記",
      vadBadge: "店主確認済み",
      pendingBadge: "未確認",
      aiBadge: "AI推定",
      confidence: "信頼度",
      texture: "食感",
      textureDrink: "口当たり",
      pairing: "合う飲み物",
      pairingDrink: "合う料理",
      howToEat: "食べ方",
      howToEatDrink: "飲み方",
      story: "この料理について",
      storyDrink: "この一杯について",
      servingStyle: "提供",
      tasteChart: "味わい",
      kidFriendly: "お子様OK",
      notKidFriendly: "お子様向けではない場合あり",
      suggestEdit: "この情報を修正",
      abv: "度数",
      tasteUmami: "旨味",
      tasteSweetness: "甘味",
      tasteSourness: "酸味",
      tasteSaltiness: "塩味",
      tasteBitterness: "苦味",
      tasteSpiciness: "辛味",
      tasteRichness: "コク",
      tasteLightness: "新鮮",
      tasteVolume: "量",
      tasteLocality: "地元",
    },
    quickExplain: {
      scanning: "解析中...",
      verified: "店主確認済み",
      pending: "登録済み",
      aiEstimate: "AI推定",
      newItem: "新メニュー",
      title: "Quick Explain ({n}品)",
    },
    feedback: {
      copy: "コピー",
      copied: "コピー済み",
      share: "シェア",
    },
    qrScanner: {
      title: "QRコードをスキャン",
      close: "閉じる",
      loading: "カメラを起動中...",
      cameraError: "カメラにアクセスできません。権限を確認してください。",
      invalidQR: "無効なQRコード形式です。",
      retry: "再試行",
      redirecting: "レストランにリダイレクト中...",
      torchOn: "ライトオン",
      torchOff: "ライトオフ",
      switchCamera: "切替",
      instructions: "レストランのQRコードにカメラを向けてください",
    },
    capture: {
      menuAnalysis: "メニュー解析結果（{n}品）",
      imageAnalyzedWith: "「{q}」の画像を解析しました。",
      imageAnalyzed: "メニュー画像を解析しました。",
      notFound: "レストランが見つかりませんでした",
      checkUrl: "URLを確認してください",
      loading: "読み込み中...",
      writeReview: "口コミを書く",
      submitReview: "口コミを投稿",
      reviewPlaceholder: "料理の感想、お店の雰囲気、おすすめポイントなど",
      submitting: "送信中...",
      submit: "投稿する",
      reviewSubmitted: "投稿しました！",
      comparing: "比較中",
      compare: "比較",
      less: "閉じる",
      moreDetails: "詳しく見る",
      relatedMenu: "関連メニュー",
      tellMeMore: "{num}の{name}について詳しく教えて",
      openMap: "地図で見る",
    },
    webLanding: {
      exploreMenu: "メニューを見る",
      makeReservation: "予約する",
      scanMenu: "メニューを撮影",
      hours: "営業時間",
      access: "アクセス",
      closed: "定休日",
    },
  },
  ko: {
    common: {
      back: "뒤로",
      reload: "새로고침",
      translate: "번역",
      openCamera: "카메라 열기",
      menu: "메뉴",
      guide: "가이드",
    },
    language: {
      change: "언어 변경",
      select: "언어 선택",
      list: "언어 목록",
      modalTitle: "언어",
      fallback: "언어",
    },
    drawer: {
      historyLabel: "기록 서랍",
    },
    home: {
      bannerLabel: "배너",
      bannerAlt: "배너",
    },
    cta: {
      go: "이동",
      continue: "계속",
    },
    captureHeader: {
      menu: "메뉴",
      scan: "번역",
      map: "지도",
    },
    cameraPrompt: {
      openCamera: "카메라 열기",
    },
    chat: {
      uploadPreview: "업로드 미리보기",
      removeAttachment: "첨부 삭제",
      tapToOpen: "탭하여 입력 열기",
      camera: "채팅 카메라",
      gallery: "채팅 갤러리",
      messageInput: "메시지 입력",
      sendMessage: "메시지 전송",
      send: "전송",
      sendDisabled: "전송 비활성",
      attachmentGuide: "AI 가이드입니다. 무엇이든 물어보세요.",
    },
    attachment: {
      cameraPhoto: "카메라 사진",
      photoLibrary: "사진 라이브러리",
    },
    camera: {
      unavailable: "카메라를 사용할 수 없습니다",
      close: "닫기",
      translate: "번역",
      openGallery: "갤러리 열기",
      capturePhoto: "사진 촬영",
      flipCamera: "카메라 전환",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "뒤로",
      closeMenu: "메뉴 닫기",
      toggleMenu: "메뉴 토글",
      nav: {
        stores: "매장",
        groups: "그룹",
        prompt: "프롬프트",
        logout: "로그아웃",
      },
      title: {
        stores: "매장",
      },
    },
    browser: {
      chromeMock: "브라우저 UI 모형",
    },
    history: {
      sectionConversation: "대화",
      sectionExplore: "메뉴 탐색",
      favorites: "즐겨찾기",
      popular: "인기 순위",
      sectionStore: "매장",
      storeInfo: "매장 정보",
      sectionPreferences: "개인 설정",
      preferencesAllergy: "알레르기·식사 스타일",
      exploreOther: "다른 매장 보기",
      title: "기록 제목...",
      brandTagline: "음식을 올바르게 전하는 데이터 인프라.",
      sidebarTitle: "기록",
      newChat: "새 대화",
      empty: "아직 대화가 없습니다",
      writeReview: "리뷰 작성하기",
    },
    hero: {
      heading: "찍고 번역",
      sub: "카메라를 눌러 시작",
    },
    explore: {
      title: " 탐색",
      subtitle: "와 함께 탐색",
      placeholder: "주변에 대해 물어보세요...",
      searchPlaceholder: "레스토랑 검색",
      allCities: "전체",
      nearbyTitle: "근처 매장",
      listTitle: "매장 목록",
      loadMore: "더 보기",
      noResults: "해당하는 레스토랑이 없습니다",
    },
    suggestions: {
      guide: "AI 가이드입니다. 무엇이든 물어보세요.",
      chips: ["어떻게 써요?", "무엇이 가능해요?", "식사 예절"],
    },
    restaurant: {
      title: "레스토랑 선택",
      subtitle: "탐색할 레스토랑을 선택하세요",
      selectPlaceholder: "레스토랑 선택...",
      continue: "계속",
      noSelection: "레스토랑을 선택해주세요",
      chatPlaceholder: "무엇이든 물어보세요",
      chatGuide: "{name}의 AI 가이드입니다. 이 레스토랑에 대해 무엇이든 물어보세요!",
      signatureDish: "여기 명물이 뭐예요?",
      aboutCuisine: "{cuisine} 요리에 대해 알려주세요",
      bestTime: "뭐가 맛있어요?",
      dietaryOptions: "못 먹는 게 있어요",
      undecided: "고민 중이에요",
      hearty: "든든하게",
      singleDish: "일품요리",
      lightAppetizer: "전채·샐러드",
      seasonalMenu: "계절 메뉴는?",
      drinksMenu: "음료는?",
      googleReview: "리뷰",
      reviewPromptText: "다음 방문객에게 도움이 되니, 괜찮으시면 리뷰도 남겨주세요 🌟",
    },
    nfg: {
      ingredients: "재료",
      allergens: "알레르기 유발 물질",
      restrictions: "식이 제한",
      flavorProfile: "맛 프로필",
      calories: "예상 칼로리",
      taxNote: "세금",
      vadBadge: "VAD 인증됨",
      aiBadge: "AI 추정",
      confidence: "신뢰도",
      texture: "식감",
      textureDrink: "입맛",
      pairing: "어울리는 음료",
      pairingDrink: "어울리는 요리",
      howToEat: "먹는 방법",
      howToEatDrink: "마시는 방법",
      story: "이 요리에 대해",
      storyDrink: "이 한잔에 대해",
      servingStyle: "제공 방식",
      tasteChart: "맛",
      kidFriendly: "어린이 적합",
      notKidFriendly: "어린이에게 적합하지 않을 수 있음",
      suggestEdit: "수정 제안",
      abv: "도수",
      tasteUmami: "감칠맛",
      tasteSweetness: "단맛",
      tasteSourness: "신맛",
      tasteSaltiness: "짠맛",
      tasteBitterness: "쓴맛",
      tasteSpiciness: "매운맛",
      tasteRichness: "깊은맛",
      tasteLightness: "신선",
      tasteVolume: "양",
      tasteLocality: "로컬",
    },
    qrScanner: {
      title: "QR 코드 스캔",
      close: "닫기",
      loading: "카메라 시작 중...",
      cameraError: "카메라에 접근할 수 없습니다. 권한을 확인하세요.",
      invalidQR: "잘못된 QR 코드 형식입니다.",
      retry: "재시도",
      redirecting: "레스토랑으로 이동 중...",
      torchOn: "조명 켜기",
      torchOff: "조명 끄기",
      switchCamera: "전환",
      instructions: "레스토랑 QR 코드에 카메라를 향하세요",
    },
    capture: {
      menuAnalysis: "메뉴 분석 결과 ({n}품)",
      imageAnalyzedWith: '"{q}" 이미지를 분석했습니다.',
      imageAnalyzed: "메뉴 이미지를 분석했습니다.",
      notFound: "레스토랑을 찾을 수 없습니다",
      checkUrl: "URL을 확인해주세요",
      loading: "로딩 중...",
      writeReview: "리뷰 작성",
      submitReview: "리뷰 작성",
      reviewPlaceholder: "음식 감상, 분위기, 추천 포인트 등",
      submitting: "전송 중...",
      submit: "게시",
      reviewSubmitted: "게시되었습니다!",
      comparing: "비교 중",
      compare: "비교",
      less: "닫기",
      moreDetails: "자세히 보기",
      relatedMenu: "관련 메뉴",
      tellMeMore: "#{num} {name}에 대해 자세히 알려주세요",
      openMap: "지도",
    },
    webLanding: {
      exploreMenu: "메뉴 보기",
      makeReservation: "예약하기",
      scanMenu: "메뉴 촬영",
      hours: "영업시간",
      access: "오시는 길",
      closed: "휴무일",
    },
  },
  "zh-Hans": {
    common: {
      back: "返回",
      reload: "刷新",
      translate: "翻译",
      openCamera: "打开相机",
      menu: "菜单",
      guide: "指南",
    },
    language: {
      change: "更改语言",
      select: "选择语言",
      list: "语言列表",
      modalTitle: "语言",
      fallback: "语言",
    },
    drawer: {
      historyLabel: "历史抽屉",
    },
    home: {
      bannerLabel: "横幅",
      bannerAlt: "横幅",
    },
    cta: {
      go: "去",
      continue: "继续",
    },
    captureHeader: {
      menu: "菜单",
      scan: "扫描",
      map: "地图",
    },
    cameraPrompt: {
      openCamera: "打开相机",
    },
    chat: {
      uploadPreview: "上传预览",
      removeAttachment: "移除附件",
      tapToOpen: "点击打开输入",
      camera: "聊天相机",
      gallery: "聊天相册",
      messageInput: "消息输入",
      sendMessage: "发送消息",
      send: "发送",
      sendDisabled: "发送不可用",
      attachmentGuide: "你的AI向导，随时提问。",
    },
    attachment: {
      cameraPhoto: "相机照片",
      photoLibrary: "相册",
    },
    camera: {
      unavailable: "相机不可用",
      close: "关闭",
      translate: "翻译",
      openGallery: "打开相册",
      capturePhoto: "拍照",
      flipCamera: "切换相机",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "返回",
      closeMenu: "关闭菜单",
      toggleMenu: "切换菜单",
      nav: {
        stores: "门店",
        groups: "分组",
        prompt: "提示",
        logout: "退出登录",
      },
      title: {
        stores: "门店",
      },
    },
    browser: {
      chromeMock: "浏览器外观模拟",
    },
    history: {
      sectionConversation: "对话",
      sectionExplore: "探索菜单",
      favorites: "收藏",
      popular: "人气排行",
      sectionStore: "店铺",
      storeInfo: "店铺信息",
      sectionPreferences: "个人设置",
      preferencesAllergy: "过敏·饮食习惯",
      exploreOther: "查看其他店铺",
      title: "历史标题...",
      brandTagline: "准确传达美食的数据基础设施。",
      sidebarTitle: "历史记录",
      newChat: "新对话",
      empty: "还没有对话",
      writeReview: "写评价",
    },
    hero: {
      heading: "拍照翻译",
      sub: "点击相机开始",
    },
    explore: {
      title: "探索 ",
      subtitle: "与  一起探索",
      placeholder: "询问附近...",
      searchPlaceholder: "搜索餐厅",
      allCities: "全部",
      nearbyTitle: "附近餐厅",
      listTitle: "餐厅列表",
      loadMore: "加载更多",
      noResults: "未找到餐厅",
    },
    suggestions: {
      guide: "你的AI向导，随时提问。",
      chips: ["如何使用？", "能做什么？", "用餐礼仪"],
    },
    restaurant: {
      title: "Select Restaurant",
      subtitle: "Choose a restaurant to explore",
      selectPlaceholder: "Select a restaurant...",
      continue: "Continue",
      noSelection: "Please select a restaurant",
      chatPlaceholder: "随便问我吧",
      chatGuide: "{name}的AI向导，随时询问关于这家餐厅的任何问题！",
      signatureDish: "这里的招牌是什么？",
      aboutCuisine: "告诉我关于{cuisine}菜的信息",
      bestTime: "有什么推荐的？",
      dietaryOptions: "我有忌口",
      undecided: "我在纠结",
      hearty: "吃饱",
      singleDish: "单品",
      lightAppetizer: "前菜·沙拉",
      seasonalMenu: "季节菜单？",
      drinksMenu: "饮品？",
      googleReview: "评价",
      reviewPromptText: "您的评价能帮助下一位访客 🌟",
    },
    nfg: {
      ingredients: "食材",
      allergens: "过敏原",
      restrictions: "饮食限制",
      flavorProfile: "风味特点",
      calories: "估计热量",
      taxNote: "含税",
      vadBadge: "VAD 已验证",
      aiBadge: "AI 估计",
      confidence: "可信度",
      texture: "口感",
      textureDrink: "口感",
      pairing: "搭配推荐",
      pairingDrink: "搭配菜肴",
      howToEat: "建议吃法",
      howToEatDrink: "饮用方法",
      story: "关于这道菜",
      storyDrink: "关于这杯酒",
      servingStyle: "供餐方式",
      tasteChart: "味道",
      kidFriendly: "适合儿童",
      notKidFriendly: "可能不适合儿童",
      suggestEdit: "建议修改",
      abv: "酒精度",
      tasteUmami: "鲜味",
      tasteSweetness: "甜味",
      tasteSourness: "酸味",
      tasteSaltiness: "咸味",
      tasteBitterness: "苦味",
      tasteSpiciness: "辣味",
      tasteRichness: "浓郁",
      tasteLightness: "清爽",
      tasteVolume: "量",
      tasteLocality: "本地",
    },
    capture: {
      menuAnalysis: "菜单分析结果（{n}品）",
      imageAnalyzedWith: '已分析"{q}"的图片。',
      imageAnalyzed: "已分析菜单图片。",
      notFound: "未找到餐厅",
      checkUrl: "请检查URL",
      loading: "加载中...",
      writeReview: "写评价",
      submitReview: "写评价",
      reviewPlaceholder: "料理感想、氛围、推荐要点等",
      submitting: "发送中...",
      submit: "提交",
      reviewSubmitted: "已提交！",
      comparing: "比较中",
      compare: "比较",
      less: "收起",
      moreDetails: "查看详情",
      relatedMenu: "相关菜单",
      tellMeMore: "请告诉我更多关于#{num} {name}",
      openMap: "地图",
    },
    webLanding: {
      exploreMenu: "浏览菜单",
      makeReservation: "预约",
      scanMenu: "拍摄菜单",
      hours: "营业时间",
      access: "交通",
      closed: "休息日",
    },
  },
  "zh-Hant": {
    common: {
      back: "返回",
      reload: "重新整理",
      translate: "翻譯",
      openCamera: "開啟相機",
      menu: "選單",
      guide: "指南",
    },
    language: {
      change: "更改語言",
      select: "選擇語言",
      list: "語言清單",
      modalTitle: "語言",
      fallback: "語言",
    },
    drawer: {
      historyLabel: "歷史抽屜",
    },
    home: {
      bannerLabel: "橫幅",
      bannerAlt: "橫幅",
    },
    cta: {
      go: "前往",
      continue: "繼續",
    },
    captureHeader: {
      menu: "選單",
      scan: "掃描",
      map: "地圖",
    },
    cameraPrompt: {
      openCamera: "開啟相機",
    },
    chat: {
      uploadPreview: "上傳預覽",
      removeAttachment: "移除附件",
      tapToOpen: "點擊開啟輸入",
      camera: "聊天相機",
      gallery: "聊天相簿",
      messageInput: "訊息輸入",
      sendMessage: "發送訊息",
      send: "發送",
      sendDisabled: "發送已停用",
      attachmentGuide: "你的 AI 導覽，隨時提問。",
    },
    attachment: {
      cameraPhoto: "相機照片",
      photoLibrary: "相簿",
    },
    camera: {
      unavailable: "相機無法使用",
      close: "關閉",
      translate: "翻譯",
      openGallery: "開啟相簿",
      capturePhoto: "拍照",
      flipCamera: "切換相機",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "返回",
      closeMenu: "關閉選單",
      toggleMenu: "切換選單",
      nav: {
        stores: "門市",
        groups: "群組",
        prompt: "提示",
        logout: "登出",
      },
      title: {
        stores: "門市",
      },
    },
    browser: {
      chromeMock: "瀏覽器介面模擬",
    },
    history: {
      sectionConversation: "對話",
      sectionExplore: "探索菜單",
      favorites: "收藏",
      popular: "人氣排行",
      sectionStore: "店鋪",
      storeInfo: "店鋪資訊",
      sectionPreferences: "個人設定",
      preferencesAllergy: "過敏·飲食習慣",
      exploreOther: "查看其他店鋪",
      title: "歷史標題...",
      brandTagline: "準確傳達美食的數據基礎設施。",
      sidebarTitle: "歷史紀錄",
      newChat: "新對話",
      empty: "還沒有對話",
      writeReview: "寫評價",
    },
    hero: {
      heading: "拍照翻譯",
      sub: "點擊相機開始",
    },
    explore: {
      title: "探索 ",
      subtitle: "與  一起探索",
      placeholder: "詢問附近...",
      searchPlaceholder: "搜尋餐廳",
      allCities: "全部",
      nearbyTitle: "附近餐廳",
      listTitle: "餐廳列表",
      loadMore: "載入更多",
      noResults: "未找到餐廳",
    },
    suggestions: {
      guide: "你的 AI 導覽，隨時提問。",
      chips: ["如何使用？", "能做什麼？", "用餐禮儀"],
    },
    restaurant: {
      title: "Select Restaurant",
      subtitle: "Choose a restaurant to explore",
      selectPlaceholder: "Select a restaurant...",
      continue: "Continue",
      noSelection: "Please select a restaurant",
      chatPlaceholder: "隨便問我吧",
      chatGuide: "{name}的AI導覽，隨時詢問關於這家餐廳的任何問題！",
      signatureDish: "這裡的招牌是什麼？",
      aboutCuisine: "告訴我關於{cuisine}菜的資訊",
      bestTime: "有什麼推薦的？",
      dietaryOptions: "我有忌口",
      undecided: "我在糾結",
      hearty: "吃飽",
      singleDish: "單品",
      lightAppetizer: "前菜·沙拉",
      seasonalMenu: "季節菜單？",
      drinksMenu: "飲品？",
      googleReview: "評價",
      reviewPromptText: "您的評價能幫助下一位訪客 🌟",
    },
    nfg: {
      ingredients: "食材",
      allergens: "過敏原",
      restrictions: "飲食限制",
      flavorProfile: "風味特點",
      calories: "估計熱量",
      taxNote: "含稅",
      vadBadge: "VAD 已驗證",
      aiBadge: "AI 估計",
      confidence: "可信度",
      texture: "口感",
      textureDrink: "口感",
      pairing: "搭配推薦",
      pairingDrink: "搭配菜餚",
      howToEat: "建議吃法",
      howToEatDrink: "飲用方法",
      story: "關於這道菜",
      storyDrink: "關於這杯酒",
      servingStyle: "供餐方式",
      tasteChart: "味道",
      kidFriendly: "適合兒童",
      notKidFriendly: "可能不適合兒童",
      suggestEdit: "建議修改",
      abv: "酒精度",
      tasteUmami: "鮮味",
      tasteSweetness: "甜味",
      tasteSourness: "酸味",
      tasteSaltiness: "鹹味",
      tasteBitterness: "苦味",
      tasteSpiciness: "辣味",
      tasteRichness: "濃郁",
      tasteLightness: "清爽",
      tasteVolume: "量",
      tasteLocality: "本地",
    },
    capture: {
      menuAnalysis: "菜單分析結果（{n}品）",
      imageAnalyzedWith: '已分析「{q}」的圖片。',
      imageAnalyzed: "已分析菜單圖片。",
      notFound: "找不到餐廳",
      checkUrl: "請檢查URL",
      loading: "載入中...",
      writeReview: "寫評價",
      submitReview: "寫評價",
      reviewPlaceholder: "料理感想、氣氛、推薦要點等",
      submitting: "傳送中...",
      submit: "提交",
      reviewSubmitted: "已提交！",
      comparing: "比較中",
      compare: "比較",
      less: "收起",
      moreDetails: "查看詳情",
      relatedMenu: "相關菜單",
      tellMeMore: "請告訴我更多關於#{num} {name}",
      openMap: "地圖",
    },
  },
  es: {
    common: {
      back: "Atrás",
      reload: "Recargar",
      translate: "Traducir",
      openCamera: "Abrir cámara",
      menu: "Menú",
      guide: "Guía",
    },
    language: {
      change: "Cambiar idioma",
      select: "Seleccionar idioma",
      list: "Lista de idiomas",
      modalTitle: "Idioma",
      fallback: "Idioma",
    },
    drawer: {
      historyLabel: "Cajón de historial",
    },
    home: {
      bannerLabel: "Banner",
      bannerAlt: "Banner",
    },
    cta: {
      go: "Ir",
      continue: "Continuar",
    },
    captureHeader: {
      menu: "Menú",
      scan: "Escanear",
    },
    cameraPrompt: {
      openCamera: "Abrir cámara",
    },
    chat: {
      uploadPreview: "Vista previa de carga",
      removeAttachment: "Quitar adjunto",
      tapToOpen: "Toca para abrir la entrada",
      camera: "Cámara de chat",
      gallery: "Galería de chat",
      messageInput: "Entrada de mensaje",
      sendMessage: "Enviar mensaje",
      send: "Enviar",
      sendDisabled: "Envío deshabilitado",
      attachmentGuide: "Tu guía de IA. Pregunta lo que quieras.",
    },
    attachment: {
      cameraPhoto: "Foto de cámara",
      photoLibrary: "Biblioteca de fotos",
    },
    camera: {
      unavailable: "Cámara no disponible",
      close: "Cerrar",
      translate: "Traducir",
      openGallery: "Abrir galería",
      capturePhoto: "Tomar foto",
      flipCamera: "Cambiar cámara",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "Atrás",
      closeMenu: "Cerrar menú",
      toggleMenu: "Alternar menú",
      nav: {
        stores: "Tiendas",
        groups: "Grupos",
        prompt: "Prompt",
        logout: "Cerrar sesión",
      },
      title: {
        stores: "Tiendas",
      },
    },
    browser: {
      chromeMock: "Maqueta de navegador",
    },
    history: {
      sectionConversation: "Conversaciones",
      newChat: "Nuevo chat",
      empty: "Aún no hay conversaciones",
      sectionExplore: "Explorar menú",
      favorites: "Favoritos",
      popular: "Populares",
      sectionStore: "Restaurante",
      storeInfo: "Información del restaurante",
      sectionPreferences: "Preferencias",
      preferencesAllergy: "Alergias y dieta",
      exploreOther: "Ver otros restaurantes",
      title: "Título de historial...",
    },
    hero: {
      heading: "Foto y traduce",
      sub: "Toca la cámara para empezar",
    },
    explore: {
      title: "Explora ",
      subtitle: "Explora con ",
      placeholder: "Pregunta sobre la zona...",
      searchPlaceholder: "Search restaurants",
      allCities: "All",
      nearbyTitle: "Nearby",
      listTitle: "Restaurants",
      loadMore: "Load more",
      noResults: "No restaurants found",
    },
    suggestions: {
      guide: "Tu guía de IA. Pregunta lo que quieras.",
      chips: ["¿Cómo se usa?", "¿Qué puede hacer?", "Etiqueta en la mesa"],
    },
    restaurant: {
      title: "Select Restaurant",
      subtitle: "Choose a restaurant to explore",
      selectPlaceholder: "Select a restaurant...",
      continue: "Continue",
      noSelection: "Please select a restaurant",
      chatPlaceholder: "Pregúntame lo que quieras",
      chatGuide: "Guía de IA para {name} — ¡Pregúntame cualquier cosa sobre este restaurante!",
      signatureDish: "¿Cuál es la especialidad?",
      aboutCuisine: "Cuéntame sobre la cocina {cuisine}",
      bestTime: "¿Qué me recomiendas?",
      dietaryOptions: "Tengo restricciones alimentarias",
      undecided: "No me decido",
      hearty: "Plato fuerte",
      singleDish: "Un plato",
      lightAppetizer: "Entrante / Ensalada",
      seasonalMenu: "¿Menú de temporada?",
      drinksMenu: "¿Bebidas?",
      googleReview: "Reseña",
    },
    nfg: {
      ingredients: "Ingredientes",
      allergens: "Alérgenos",
      restrictions: "Restricciones dietéticas",
      flavorProfile: "Perfil de sabor",
      calories: "Calorías estimadas",
      taxNote: "Impuesto",
      vadBadge: "VAD Verificado",
      aiBadge: "Estimación IA",
      confidence: "Confiabilidad",
      texture: "Textura",
      pairing: "Combina con",
      howToEat: "Cómo comer",
      story: "Historia",
      servingStyle: "Servicio",
      tasteChart: "Sabor",
      kidFriendly: "Apto para niños",
      notKidFriendly: "Puede no ser apto para niños",
      suggestEdit: "Sugerir edición",
      abv: "ABV",
      tasteUmami: "Umami",
      tasteSweetness: "Dulce",
      tasteSourness: "Ácido",
      tasteSaltiness: "Salado",
      tasteBitterness: "Amargo",
      tasteSpiciness: "Picante",
      tasteRichness: "Rico",
      tasteLightness: "Fresco",
      tasteVolume: "Volumen",
      tasteLocality: "Local",
    },
    capture: {
      menuAnalysis: "Menu Analysis ({n} items)",
      imageAnalyzedWith: 'Analyzed image of "{q}".',
      imageAnalyzed: "Menu image analyzed.",
      notFound: "Restaurant not found",
      checkUrl: "Please check the URL",
      loading: "Loading...",
      writeReview: "Write a review",
      submitReview: "Write a review",
      reviewPlaceholder: "Food impressions, atmosphere, recommendations...",
      submitting: "Sending...",
      submit: "Submit",
      reviewSubmitted: "Review submitted!",
      comparing: "Comparing",
      compare: "Compare",
      less: "Less",
      moreDetails: "More details",
      relatedMenu: "Related",
      tellMeMore: "Tell me more about #{num} {name}",
      openMap: "Map",
    },
  },
  fr: {
    common: {
      back: "Retour",
      reload: "Recharger",
      translate: "Traduire",
      openCamera: "Ouvrir la caméra",
      menu: "Menu",
      guide: "Guide",
    },
    language: {
      change: "Changer de langue",
      select: "Sélectionner la langue",
      list: "Liste des langues",
      modalTitle: "Langue",
      fallback: "Langue",
    },
    drawer: {
      historyLabel: "Tiroir d'historique",
    },
    home: {
      bannerLabel: "Bannière",
      bannerAlt: "Bannière",
    },
    cta: {
      go: "Aller",
      continue: "Continuer",
    },
    captureHeader: {
      menu: "Menu",
      scan: "Scanner",
    },
    cameraPrompt: {
      openCamera: "Ouvrir la caméra",
    },
    chat: {
      uploadPreview: "Aperçu du téléversement",
      removeAttachment: "Retirer la pièce jointe",
      tapToOpen: "Touchez pour ouvrir la saisie",
      camera: "Caméra du chat",
      gallery: "Galerie du chat",
      messageInput: "Saisie de message",
      sendMessage: "Envoyer le message",
      send: "Envoyer",
      sendDisabled: "Envoi désactivé",
      attachmentGuide: "Votre guide IA. Demandez-moi n'importe quoi.",
    },
    attachment: {
      cameraPhoto: "Photo de la caméra",
      photoLibrary: "Photothèque",
    },
    camera: {
      unavailable: "Caméra indisponible",
      close: "Fermer",
      translate: "Traduire",
      openGallery: "Ouvrir la galerie",
      capturePhoto: "Prendre une photo",
      flipCamera: "Changer de caméra",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "Retour",
      closeMenu: "Fermer le menu",
      toggleMenu: "Basculer le menu",
      nav: {
        stores: "Boutiques",
        groups: "Groupes",
        prompt: "Prompt",
        logout: "Déconnexion",
      },
      title: {
        stores: "Boutiques",
      },
    },
    browser: {
      chromeMock: "Maquette de navigateur",
    },
    history: {
      sectionConversation: "Conversations",
      newChat: "Nouveau chat",
      empty: "Aucune conversation pour l'instant",
      sectionExplore: "Explorer le menu",
      favorites: "Favoris",
      popular: "Populaires",
      sectionStore: "Restaurant",
      storeInfo: "Infos du restaurant",
      sectionPreferences: "Préférences",
      preferencesAllergy: "Allergies et régime",
      exploreOther: "Voir d'autres restaurants",
      title: "Titre d'historique...",
    },
    hero: {
      heading: "Prenez et traduisez",
      sub: "Touchez la caméra pour commencer",
    },
    explore: {
      title: "Explorer ",
      subtitle: "Explorez avec ",
      placeholder: "Demandez à propos des environs...",
      searchPlaceholder: "Search restaurants",
      allCities: "All",
      nearbyTitle: "Nearby",
      listTitle: "Restaurants",
      loadMore: "Load more",
      noResults: "No restaurants found",
    },
    suggestions: {
      guide: "Votre guide IA. Demandez-moi n'importe quoi.",
      chips: ["Comment l'utiliser ?", "Que peut-il faire ?", "Étiquette à table"],
    },
    restaurant: {
      title: "Select Restaurant",
      subtitle: "Choose a restaurant to explore",
      selectPlaceholder: "Select a restaurant...",
      continue: "Continue",
      noSelection: "Please select a restaurant",
      chatPlaceholder: "Demandez-moi n'importe quoi",
      chatGuide: "Guide IA pour {name} — Demandez-moi n'importe quoi sur ce restaurant !",
      signatureDish: "C'est quoi la spécialité ?",
      aboutCuisine: "Parlez-moi de la cuisine {cuisine}",
      bestTime: "Qu'est-ce que vous recommandez ?",
      dietaryOptions: "J'ai des restrictions alimentaires",
      undecided: "J'hésite",
      hearty: "Plat copieux",
      singleDish: "Un plat",
      lightAppetizer: "Entrée / Salade",
      seasonalMenu: "Menu de saison ?",
      drinksMenu: "Boissons ?",
      googleReview: "Avis",
    },
    nfg: {
      ingredients: "Ingrédients",
      allergens: "Allergènes",
      restrictions: "Restrictions alimentaires",
      flavorProfile: "Profil gustatif",
      calories: "Calories estimées",
      taxNote: "Taxe",
      vadBadge: "VAD Vérifié",
      aiBadge: "Estimation IA",
      confidence: "Fiabilité",
      texture: "Texture",
      pairing: "Se marie avec",
      howToEat: "Comment manger",
      story: "Histoire",
      servingStyle: "Service",
      tasteChart: "Goût",
      kidFriendly: "Adapté aux enfants",
      notKidFriendly: "Peut ne pas convenir aux enfants",
      suggestEdit: "Suggérer une modification",
      abv: "ABV",
      tasteUmami: "Umami",
      tasteSweetness: "Sucré",
      tasteSourness: "Acide",
      tasteSaltiness: "Salé",
      tasteBitterness: "Amer",
      tasteSpiciness: "Épicé",
      tasteRichness: "Riche",
      tasteLightness: "Frais",
      tasteVolume: "Volume",
      tasteLocality: "Local",
    },
    capture: {
      menuAnalysis: "Menu Analysis ({n} items)",
      imageAnalyzedWith: 'Analyzed image of "{q}".',
      imageAnalyzed: "Menu image analyzed.",
      notFound: "Restaurant not found",
      checkUrl: "Please check the URL",
      loading: "Loading...",
      writeReview: "Write a review",
      submitReview: "Write a review",
      reviewPlaceholder: "Food impressions, atmosphere, recommendations...",
      submitting: "Sending...",
      submit: "Submit",
      reviewSubmitted: "Review submitted!",
      comparing: "Comparing",
      compare: "Compare",
      less: "Less",
      moreDetails: "More details",
      relatedMenu: "Related",
      tellMeMore: "Tell me more about #{num} {name}",
      openMap: "Map",
    },
  },
  de: {
    common: {
      back: "Zurück",
      reload: "Neu laden",
      translate: "Übersetzen",
      openCamera: "Kamera öffnen",
      menu: "Menü",
      guide: "Leitfaden",
    },
    language: {
      change: "Sprache ändern",
      select: "Sprache auswählen",
      list: "Sprachliste",
      modalTitle: "Sprache",
      fallback: "Sprache",
    },
    drawer: {
      historyLabel: "Verlaufsschublade",
    },
    home: {
      bannerLabel: "Banner",
      bannerAlt: "Banner",
    },
    cta: {
      go: "Los",
      continue: "Weiter",
    },
    captureHeader: {
      menu: "Menü",
      scan: "Scannen",
    },
    cameraPrompt: {
      openCamera: "Kamera öffnen",
    },
    chat: {
      uploadPreview: "Upload-Vorschau",
      removeAttachment: "Anhang entfernen",
      tapToOpen: "Tippen, um die Eingabe zu öffnen",
      camera: "Chat-Kamera",
      gallery: "Chat-Galerie",
      messageInput: "Nachrichteneingabe",
      sendMessage: "Nachricht senden",
      send: "Senden",
      sendDisabled: "Senden deaktiviert",
      attachmentGuide: "Dein KI-Guide. Frag mich alles.",
    },
    attachment: {
      cameraPhoto: "Kamerafoto",
      photoLibrary: "Fotomediathek",
    },
    camera: {
      unavailable: "Kamera nicht verfügbar",
      close: "Schließen",
      translate: "Übersetzen",
      openGallery: "Galerie öffnen",
      capturePhoto: "Foto aufnehmen",
      flipCamera: "Kamera wechseln",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "Zurück",
      closeMenu: "Menü schließen",
      toggleMenu: "Menü umschalten",
      nav: {
        stores: "Filialen",
        groups: "Gruppen",
        prompt: "Prompt",
        logout: "Abmelden",
      },
      title: {
        stores: "Filialen",
      },
    },
    browser: {
      chromeMock: "Browser-UI-Mock",
    },
    history: {
      sectionConversation: "Unterhaltungen",
      newChat: "Neuer Chat",
      empty: "Noch keine Unterhaltungen",
      sectionExplore: "Menü entdecken",
      favorites: "Favoriten",
      popular: "Beliebt",
      sectionStore: "Restaurant",
      storeInfo: "Restaurant-Info",
      sectionPreferences: "Einstellungen",
      preferencesAllergy: "Allergien & Ernährung",
      exploreOther: "Andere Restaurants ansehen",
      title: "Verlaufstitel...",
    },
    hero: {
      heading: "Knipsen & Übersetzen",
      sub: "Tippe auf die Kamera, um zu starten",
    },
    explore: {
      title: " entdecken",
      subtitle: "Entdecke mit ",
      placeholder: "Frag nach der Umgebung...",
      searchPlaceholder: "Search restaurants",
      allCities: "All",
      nearbyTitle: "Nearby",
      listTitle: "Restaurants",
      loadMore: "Load more",
      noResults: "No restaurants found",
    },
    suggestions: {
      guide: "Dein KI-Guide. Frag mich alles.",
      chips: ["Wie nutzt man es?", "Was kann es?", "Tischmanieren"],
    },
    restaurant: {
      title: "Select Restaurant",
      subtitle: "Choose a restaurant to explore",
      selectPlaceholder: "Select a restaurant...",
      continue: "Continue",
      noSelection: "Please select a restaurant",
      chatPlaceholder: "Frag mich einfach",
      chatGuide: "KI-Guide für {name} — Frag mich alles über dieses Restaurant!",
      signatureDish: "Was ist die Spezialität?",
      aboutCuisine: "Erzähl mir über die {cuisine}-Küche",
      bestTime: "Was empfehlen Sie?",
      dietaryOptions: "Ich habe Unverträglichkeiten",
      undecided: "Ich bin unentschlossen",
      hearty: "Herzhaft",
      singleDish: "Einzelgericht",
      lightAppetizer: "Vorspeise / Salat",
      seasonalMenu: "Saisonmenü?",
      drinksMenu: "Getränke?",
      googleReview: "Bewertung",
    },
    nfg: {
      ingredients: "Zutaten",
      allergens: "Allergene",
      restrictions: "Ernährungseinschränkungen",
      flavorProfile: "Geschmacksprofil",
      calories: "Geschätzte Kalorien",
      taxNote: "Steuer",
      vadBadge: "VAD Verifiziert",
      aiBadge: "KI-Schätzung",
      confidence: "Zuverlässigkeit",
      texture: "Textur",
      pairing: "Passt zu",
      howToEat: "Wie man isst",
      story: "Geschichte",
      servingStyle: "Servierung",
      tasteChart: "Geschmack",
      kidFriendly: "Kinderfreundlich",
      notKidFriendly: "Möglicherweise nicht für Kinder geeignet",
      suggestEdit: "Änderung vorschlagen",
      abv: "ABV",
      tasteUmami: "Umami",
      tasteSweetness: "Süß",
      tasteSourness: "Sauer",
      tasteSaltiness: "Salzig",
      tasteBitterness: "Bitter",
      tasteSpiciness: "Scharf",
      tasteRichness: "Reichhaltig",
      tasteLightness: "Frisch",
      tasteVolume: "Volumen",
      tasteLocality: "Lokal",
    },
    capture: {
      menuAnalysis: "Menu Analysis ({n} items)",
      imageAnalyzedWith: 'Analyzed image of "{q}".',
      imageAnalyzed: "Menu image analyzed.",
      notFound: "Restaurant not found",
      checkUrl: "Please check the URL",
      loading: "Loading...",
      writeReview: "Write a review",
      submitReview: "Write a review",
      reviewPlaceholder: "Food impressions, atmosphere, recommendations...",
      submitting: "Sending...",
      submit: "Submit",
      reviewSubmitted: "Review submitted!",
      comparing: "Comparing",
      compare: "Compare",
      less: "Less",
      moreDetails: "More details",
      relatedMenu: "Related",
      tellMeMore: "Tell me more about #{num} {name}",
      openMap: "Map",
    },
  },
  it: {
    common: {
      back: "Indietro",
      reload: "Ricarica",
      translate: "Traduci",
      openCamera: "Apri fotocamera",
      menu: "Menu",
      guide: "Guida",
    },
    language: {
      change: "Cambia lingua",
      select: "Seleziona lingua",
      list: "Elenco lingue",
      modalTitle: "Lingua",
      fallback: "Lingua",
    },
    drawer: {
      historyLabel: "Cassetto cronologia",
    },
    home: {
      bannerLabel: "Banner",
      bannerAlt: "Banner",
    },
    cta: {
      go: "Vai",
      continue: "Continua",
    },
    captureHeader: {
      menu: "Menu",
      scan: "Scansiona",
    },
    cameraPrompt: {
      openCamera: "Apri fotocamera",
    },
    chat: {
      uploadPreview: "Anteprima caricamento",
      removeAttachment: "Rimuovi allegato",
      tapToOpen: "Tocca per aprire l'input",
      camera: "Fotocamera chat",
      gallery: "Galleria chat",
      messageInput: "Input messaggio",
      sendMessage: "Invia messaggio",
      send: "Invia",
      sendDisabled: "Invio disabilitato",
      attachmentGuide: "La tua guida AI. Chiedi qualsiasi cosa.",
    },
    attachment: {
      cameraPhoto: "Foto della fotocamera",
      photoLibrary: "Libreria foto",
    },
    camera: {
      unavailable: "Fotocamera non disponibile",
      close: "Chiudi",
      translate: "Traduci",
      openGallery: "Apri galleria",
      capturePhoto: "Scatta foto",
      flipCamera: "Cambia fotocamera",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "Indietro",
      closeMenu: "Chiudi menu",
      toggleMenu: "Alterna menu",
      nav: {
        stores: "Negozi",
        groups: "Gruppi",
        prompt: "Prompt",
        logout: "Disconnetti",
      },
      title: {
        stores: "Negozi",
      },
    },
    browser: {
      chromeMock: "Mock browser",
    },
    history: {
      sectionConversation: "Conversazioni",
      newChat: "Nuova chat",
      empty: "Ancora nessuna conversazione",
      sectionExplore: "Esplora il menu",
      favorites: "Preferiti",
      popular: "Popolari",
      sectionStore: "Ristorante",
      storeInfo: "Info ristorante",
      sectionPreferences: "Preferenze",
      preferencesAllergy: "Allergie e dieta",
      exploreOther: "Vedi altri ristoranti",
      title: "Titolo cronologia...",
    },
    hero: {
      heading: "Scatta e traduci",
      sub: "Tocca la fotocamera per iniziare",
    },
    explore: {
      title: "Esplora ",
      subtitle: "Esplora con ",
      placeholder: "Chiedi della zona...",
      searchPlaceholder: "Search restaurants",
      allCities: "All",
      nearbyTitle: "Nearby",
      listTitle: "Restaurants",
      loadMore: "Load more",
      noResults: "No restaurants found",
    },
    suggestions: {
      guide: "La tua guida AI. Chiedi qualsiasi cosa.",
      chips: ["Come si usa?", "Cosa può fare?", "Galateo a tavola"],
    },
    restaurant: {
      title: "Select Restaurant",
      subtitle: "Choose a restaurant to explore",
      selectPlaceholder: "Select a restaurant...",
      continue: "Continue",
      noSelection: "Please select a restaurant",
      chatPlaceholder: "Chiedimi qualsiasi cosa",
      chatGuide: "Guida AI per {name} — Chiedimi qualsiasi cosa su questo ristorante!",
      signatureDish: "Qual è la specialità?",
      aboutCuisine: "Raccontami della cucina {cuisine}",
      bestTime: "Cosa mi consigliate?",
      dietaryOptions: "Ho delle restrizioni alimentari",
      undecided: "Non so decidere",
      hearty: "Piatto abbondante",
      singleDish: "Un piatto",
      lightAppetizer: "Antipasto / Insalata",
      seasonalMenu: "Menu stagionale?",
      drinksMenu: "Bevande?",
      googleReview: "Recensione",
    },
    nfg: {
      ingredients: "Ingredienti",
      allergens: "Allergeni",
      restrictions: "Restrizioni alimentari",
      flavorProfile: "Profilo del gusto",
      calories: "Calorie stimate",
      taxNote: "Tasse",
      vadBadge: "VAD Verificato",
      aiBadge: "Stima AI",
      confidence: "Affidabilità",
      texture: "Consistenza",
      pairing: "Abbinamento",
      howToEat: "Come mangiare",
      story: "Storia",
      servingStyle: "Servizio",
      tasteChart: "Gusto",
      kidFriendly: "Adatto ai bambini",
      notKidFriendly: "Potrebbe non essere adatto ai bambini",
      suggestEdit: "Suggerisci modifica",
      abv: "ABV",
      tasteUmami: "Umami",
      tasteSweetness: "Dolce",
      tasteSourness: "Acido",
      tasteSaltiness: "Salato",
      tasteBitterness: "Amaro",
      tasteSpiciness: "Piccante",
      tasteRichness: "Ricco",
      tasteLightness: "Fresco",
      tasteVolume: "Volume",
      tasteLocality: "Locale",
    },
    capture: {
      menuAnalysis: "Menu Analysis ({n} items)",
      imageAnalyzedWith: 'Analyzed image of "{q}".',
      imageAnalyzed: "Menu image analyzed.",
      notFound: "Restaurant not found",
      checkUrl: "Please check the URL",
      loading: "Loading...",
      writeReview: "Write a review",
      submitReview: "Write a review",
      reviewPlaceholder: "Food impressions, atmosphere, recommendations...",
      submitting: "Sending...",
      submit: "Submit",
      reviewSubmitted: "Review submitted!",
      comparing: "Comparing",
      compare: "Compare",
      less: "Less",
      moreDetails: "More details",
      relatedMenu: "Related",
      tellMeMore: "Tell me more about #{num} {name}",
      openMap: "Map",
    },
  },
  pt: {
    common: {
      back: "Voltar",
      reload: "Recarregar",
      translate: "Traduzir",
      openCamera: "Abrir câmera",
      menu: "Menu",
      guide: "Guia",
    },
    language: {
      change: "Mudar idioma",
      select: "Selecionar idioma",
      list: "Lista de idiomas",
      modalTitle: "Idioma",
      fallback: "Idioma",
    },
    drawer: {
      historyLabel: "Gaveta de histórico",
    },
    home: {
      bannerLabel: "Banner",
      bannerAlt: "Banner",
    },
    cta: {
      go: "Ir",
      continue: "Continuar",
    },
    captureHeader: {
      menu: "Menu",
      scan: "Escanear",
    },
    cameraPrompt: {
      openCamera: "Abrir câmera",
    },
    chat: {
      uploadPreview: "Prévia de envio",
      removeAttachment: "Remover anexo",
      tapToOpen: "Toque para abrir a entrada",
      camera: "Câmera do chat",
      gallery: "Galeria do chat",
      messageInput: "Entrada de mensagem",
      sendMessage: "Enviar mensagem",
      send: "Enviar",
      sendDisabled: "Envio desativado",
      attachmentGuide: "Seu guia de IA. Pergunte qualquer coisa.",
    },
    attachment: {
      cameraPhoto: "Foto da câmera",
      photoLibrary: "Biblioteca de fotos",
    },
    camera: {
      unavailable: "Câmera indisponível",
      close: "Fechar",
      translate: "Traduzir",
      openGallery: "Abrir galeria",
      capturePhoto: "Tirar foto",
      flipCamera: "Alternar câmera",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "Voltar",
      closeMenu: "Fechar menu",
      toggleMenu: "Alternar menu",
      nav: {
        stores: "Lojas",
        groups: "Grupos",
        prompt: "Prompt",
        logout: "Sair",
      },
      title: {
        stores: "Lojas",
      },
    },
    browser: {
      chromeMock: "Mock do navegador",
    },
    history: {
      sectionConversation: "Conversas",
      newChat: "Novo chat",
      empty: "Ainda não há conversas",
      sectionExplore: "Explorar menu",
      favorites: "Favoritos",
      popular: "Populares",
      sectionStore: "Restaurante",
      storeInfo: "Informações do restaurante",
      sectionPreferences: "Preferências",
      preferencesAllergy: "Alergias e dieta",
      exploreOther: "Ver outros restaurantes",
      title: "Título do histórico...",
    },
    hero: {
      heading: "Fotografar e traduzir",
      sub: "Toque na câmera para começar",
    },
    explore: {
      title: "Explorar ",
      subtitle: "Explore com ",
      placeholder: "Pergunte sobre a área...",
      searchPlaceholder: "Search restaurants",
      allCities: "All",
      nearbyTitle: "Nearby",
      listTitle: "Restaurants",
      loadMore: "Load more",
      noResults: "No restaurants found",
    },
    suggestions: {
      guide: "Seu guia de IA. Pergunte qualquer coisa.",
      chips: ["Como usar?", "O que pode fazer?", "Etiqueta à mesa"],
    },
    restaurant: {
      title: "Select Restaurant",
      subtitle: "Choose a restaurant to explore",
      selectPlaceholder: "Select a restaurant...",
      continue: "Continue",
      noSelection: "Please select a restaurant",
      chatPlaceholder: "Pergunte o que quiser",
      chatGuide: "Guia de IA para {name} — Pergunte-me qualquer coisa sobre este restaurante!",
      signatureDish: "Qual é a especialidade?",
      aboutCuisine: "Conte-me sobre a culinária {cuisine}",
      bestTime: "O que você recomenda?",
      dietaryOptions: "Tenho restrições alimentares",
      undecided: "Estou indeciso",
      hearty: "Prato reforçado",
      singleDish: "Um prato",
      lightAppetizer: "Entrada / Salada",
      seasonalMenu: "Menu sazonal?",
      drinksMenu: "Bebidas?",
      googleReview: "Avaliação",
    },
    nfg: {
      ingredients: "Ingredientes",
      allergens: "Alérgenos",
      restrictions: "Restrições alimentares",
      flavorProfile: "Perfil de sabor",
      calories: "Calorias estimadas",
      taxNote: "Imposto",
      vadBadge: "VAD Verificado",
      aiBadge: "Estimativa IA",
      confidence: "Confiança",
      texture: "Textura",
      pairing: "Combina com",
      howToEat: "Como comer",
      story: "História",
      servingStyle: "Serviço",
      tasteChart: "Sabor",
      kidFriendly: "Para crianças",
      notKidFriendly: "Pode não ser adequado para crianças",
      suggestEdit: "Sugerir edição",
      abv: "ABV",
      tasteUmami: "Umami",
      tasteSweetness: "Doce",
      tasteSourness: "Ácido",
      tasteSaltiness: "Salgado",
      tasteBitterness: "Amargo",
      tasteSpiciness: "Picante",
      tasteRichness: "Rico",
      tasteLightness: "Fresco",
      tasteVolume: "Volume",
      tasteLocality: "Local",
    },
    capture: {
      menuAnalysis: "Menu Analysis ({n} items)",
      imageAnalyzedWith: 'Analyzed image of "{q}".',
      imageAnalyzed: "Menu image analyzed.",
      notFound: "Restaurant not found",
      checkUrl: "Please check the URL",
      loading: "Loading...",
      writeReview: "Write a review",
      submitReview: "Write a review",
      reviewPlaceholder: "Food impressions, atmosphere, recommendations...",
      submitting: "Sending...",
      submit: "Submit",
      reviewSubmitted: "Review submitted!",
      comparing: "Comparing",
      compare: "Compare",
      less: "Less",
      moreDetails: "More details",
      relatedMenu: "Related",
      tellMeMore: "Tell me more about #{num} {name}",
      openMap: "Map",
    },
  },
  ru: {
    common: {
      back: "Назад",
      reload: "Обновить",
      translate: "Перевести",
      openCamera: "Открыть камеру",
      menu: "Меню",
      guide: "Руководство",
    },
    language: {
      change: "Изменить язык",
      select: "Выбрать язык",
      list: "Список языков",
      modalTitle: "Язык",
      fallback: "Язык",
    },
    drawer: {
      historyLabel: "Панель истории",
    },
    home: {
      bannerLabel: "Баннер",
      bannerAlt: "Баннер",
    },
    cta: {
      go: "Перейти",
      continue: "Продолжить",
    },
    captureHeader: {
      menu: "Меню",
      scan: "Сканировать",
    },
    cameraPrompt: {
      openCamera: "Открыть камеру",
    },
    chat: {
      uploadPreview: "Предпросмотр загрузки",
      removeAttachment: "Удалить вложение",
      tapToOpen: "Нажмите, чтобы открыть ввод",
      camera: "Камера чата",
      gallery: "Галерея чата",
      messageInput: "Поле сообщения",
      sendMessage: "Отправить сообщение",
      send: "Отправить",
      sendDisabled: "Отправка отключена",
      attachmentGuide: "Ваш ИИ‑гид. Спрашивайте что угодно.",
    },
    attachment: {
      cameraPhoto: "Фото с камеры",
      photoLibrary: "Фотобиблиотека",
    },
    camera: {
      unavailable: "Камера недоступна",
      close: "Закрыть",
      translate: "Перевести",
      openGallery: "Открыть галерею",
      capturePhoto: "Сделать фото",
      flipCamera: "Переключить камеру",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "Назад",
      closeMenu: "Закрыть меню",
      toggleMenu: "Переключить меню",
      nav: {
        stores: "Магазины",
        groups: "Группы",
        prompt: "Промпт",
        logout: "Выйти",
      },
      title: {
        stores: "Магазины",
      },
    },
    browser: {
      chromeMock: "Макет интерфейса браузера",
    },
    history: {
      sectionConversation: "Разговоры",
      newChat: "Новый чат",
      empty: "Пока нет разговоров",
      sectionExplore: "Меню",
      favorites: "Избранное",
      popular: "Популярное",
      sectionStore: "Заведение",
      storeInfo: "О заведении",
      sectionPreferences: "Настройки",
      preferencesAllergy: "Аллергии и питание",
      exploreOther: "Другие заведения",
      title: "Заголовок истории...",
    },
    hero: {
      heading: "Снимай и переводи",
      sub: "Нажмите на камеру, чтобы начать",
    },
    explore: {
      title: "Исследуйте ",
      subtitle: "Исследуйте с ",
      placeholder: "Спросите о месте...",
      searchPlaceholder: "Search restaurants",
      allCities: "All",
      nearbyTitle: "Nearby",
      listTitle: "Restaurants",
      loadMore: "Load more",
      noResults: "No restaurants found",
    },
    suggestions: {
      guide: "Ваш ИИ‑гид. Спрашивайте что угодно.",
      chips: ["Как пользоваться?", "Что умеет?", "Этикет за столом"],
    },
    restaurant: {
      title: "Select Restaurant",
      subtitle: "Choose a restaurant to explore",
      selectPlaceholder: "Select a restaurant...",
      continue: "Continue",
      noSelection: "Please select a restaurant",
      chatPlaceholder: "Спрашивайте что угодно",
      chatGuide: "ИИ‑гид для {name} — Спрашивайте всё об этом ресторане!",
      signatureDish: "Что тут фирменное?",
      aboutCuisine: "Расскажите о кухне {cuisine}",
      bestTime: "Что посоветуете?",
      dietaryOptions: "У меня ограничения в питании",
      undecided: "Не могу выбрать",
      hearty: "Сытное блюдо",
      singleDish: "Одно блюдо",
      lightAppetizer: "Закуска / Салат",
      seasonalMenu: "Сезонное меню?",
      drinksMenu: "Напитки?",
      googleReview: "Отзыв",
    },
    nfg: {
      ingredients: "Ингредиенты",
      allergens: "Аллергены",
      restrictions: "Диетические ограничения",
      flavorProfile: "Вкусовой профиль",
      calories: "Примерные калории",
      taxNote: "Налог",
      vadBadge: "VAD Проверено",
      aiBadge: "Оценка ИИ",
      confidence: "Достоверность",
      texture: "Текстура",
      pairing: "Сочетается с",
      howToEat: "Как есть",
      story: "История",
      servingStyle: "Подача",
      tasteChart: "Вкус",
      kidFriendly: "Подходит детям",
      notKidFriendly: "Может не подойти детям",
      suggestEdit: "Предложить правку",
      abv: "ABV",
      tasteUmami: "Умами",
      tasteSweetness: "Сладкий",
      tasteSourness: "Кислый",
      tasteSaltiness: "Солёный",
      tasteBitterness: "Горький",
      tasteSpiciness: "Острый",
      tasteRichness: "Насыщенный",
      tasteLightness: "Свежий",
      tasteVolume: "Объём",
      tasteLocality: "Местный",
    },
    capture: {
      menuAnalysis: "Menu Analysis ({n} items)",
      imageAnalyzedWith: 'Analyzed image of "{q}".',
      imageAnalyzed: "Menu image analyzed.",
      notFound: "Restaurant not found",
      checkUrl: "Please check the URL",
      loading: "Loading...",
      writeReview: "Write a review",
      submitReview: "Write a review",
      reviewPlaceholder: "Food impressions, atmosphere, recommendations...",
      submitting: "Sending...",
      submit: "Submit",
      reviewSubmitted: "Review submitted!",
      comparing: "Comparing",
      compare: "Compare",
      less: "Less",
      moreDetails: "More details",
      relatedMenu: "Related",
      tellMeMore: "Tell me more about #{num} {name}",
      openMap: "Map",
    },
  },
  th: {
    common: {
      back: "กลับ",
      reload: "โหลดใหม่",
      translate: "แปล",
      openCamera: "เปิดกล้อง",
      menu: "เมนู",
      guide: "คู่มือ",
    },
    language: {
      change: "เปลี่ยนภาษา",
      select: "เลือกภาษา",
      list: "รายการภาษา",
      modalTitle: "ภาษา",
      fallback: "ภาษา",
    },
    drawer: {
      historyLabel: "ลิ้นชักประวัติ",
    },
    home: {
      bannerLabel: "แบนเนอร์",
      bannerAlt: "แบนเนอร์",
    },
    cta: {
      go: "ไป",
      continue: "ดำเนินต่อ",
    },
    captureHeader: {
      menu: "เมนู",
      scan: "สแกน",
    },
    cameraPrompt: {
      openCamera: "เปิดกล้อง",
    },
    chat: {
      uploadPreview: "ตัวอย่างอัปโหลด",
      removeAttachment: "ลบไฟล์แนบ",
      tapToOpen: "แตะเพื่อเปิดช่องพิมพ์",
      camera: "กล้องแชท",
      gallery: "แกลเลอรีแชท",
      messageInput: "ช่องพิมพ์ข้อความ",
      sendMessage: "ส่งข้อความ",
      send: "ส่ง",
      sendDisabled: "ส่งไม่ได้",
      attachmentGuide: "ผู้ช่วย AI ของคุณ ถามได้ทุกอย่าง",
    },
    attachment: {
      cameraPhoto: "ภาพจากกล้อง",
      photoLibrary: "คลังรูปภาพ",
    },
    camera: {
      unavailable: "ไม่สามารถใช้กล้องได้",
      close: "ปิด",
      translate: "แปล",
      openGallery: "เปิดแกลเลอรี",
      capturePhoto: "ถ่ายภาพ",
      flipCamera: "สลับกล้อง",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "กลับ",
      closeMenu: "ปิดเมนู",
      toggleMenu: "สลับเมนู",
      nav: {
        stores: "ร้านค้า",
        groups: "กลุ่ม",
        prompt: "พรอมต์",
        logout: "ออกจากระบบ",
      },
      title: {
        stores: "ร้านค้า",
      },
    },
    browser: {
      chromeMock: "แบบจำลองเบราว์เซอร์",
    },
    history: {
      sectionConversation: "การสนทนา",
      newChat: "แชทใหม่",
      empty: "ยังไม่มีการสนทนา",
      sectionExplore: "สำรวจเมนู",
      favorites: "รายการโปรด",
      popular: "ยอดนิยม",
      sectionStore: "ร้าน",
      storeInfo: "ข้อมูลร้าน",
      sectionPreferences: "การตั้งค่า",
      preferencesAllergy: "ภูมิแพ้และรูปแบบอาหาร",
      exploreOther: "ดูร้านอื่น",
      title: "ชื่อประวัติ...",
    },
    hero: {
      heading: "ถ่ายแล้วแปล",
      sub: "แตะกล้องเพื่อเริ่ม",
    },
    explore: {
      title: "สำรวจ ",
      subtitle: "สำรวจกับ ",
      placeholder: "ถามเกี่ยวกับบริเวณนี้...",
      searchPlaceholder: "Search restaurants",
      allCities: "All",
      nearbyTitle: "Nearby",
      listTitle: "Restaurants",
      loadMore: "Load more",
      noResults: "No restaurants found",
    },
    suggestions: {
      guide: "ผู้ช่วย AI ของคุณ ถามได้ทุกอย่าง",
      chips: ["ใช้อย่างไร?", "ทำอะไรได้บ้าง?", "มารยาทบนโต๊ะอาหาร"],
    },
    restaurant: {
      title: "Select Restaurant",
      subtitle: "Choose a restaurant to explore",
      selectPlaceholder: "Select a restaurant...",
      continue: "Continue",
      noSelection: "Please select a restaurant",
      chatPlaceholder: "ถามอะไรก็ได้เลย",
      chatGuide: "ผู้ช่วย AI สำหรับ {name} — ถามอะไรก็ได้เกี่ยวกับร้านอาหารนี้!",
      signatureDish: "ของเด่นที่นี่คืออะไร?",
      aboutCuisine: "บอกเกี่ยวกับอาหาร {cuisine}",
      bestTime: "แนะนำอะไรดี?",
      dietaryOptions: "มีของที่กินไม่ได้",
      undecided: "ยังตัดสินใจไม่ได้",
      hearty: "จานหนัก",
      singleDish: "จานเดียว",
      lightAppetizer: "ออเดิร์ฟ / สลัด",
      seasonalMenu: "เมนูตามฤดูกาล?",
      drinksMenu: "เครื่องดื่ม?",
      googleReview: "รีวิว",
    },
    nfg: {
      ingredients: "ส่วนผสม",
      allergens: "สารก่อภูมิแพ้",
      restrictions: "ข้อจำกัดด้านอาหาร",
      flavorProfile: "โปรไฟล์รสชาติ",
      calories: "แคลอรี่โดยประมาณ",
      taxNote: "ภาษี",
      vadBadge: "VAD ยืนยันแล้ว",
      aiBadge: "AI ประมาณการ",
      confidence: "ความน่าเชื่อถือ",
      texture: "เนื้อสัมผัส",
      pairing: "เข้ากันกับ",
      howToEat: "วิธีกิน",
      story: "เรื่องราว",
      servingStyle: "การเสิร์ฟ",
      tasteChart: "รสชาติ",
      kidFriendly: "เหมาะสำหรับเด็ก",
      notKidFriendly: "อาจไม่เหมาะสำหรับเด็ก",
      suggestEdit: "แนะนำการแก้ไข",
      abv: "ABV",
      tasteUmami: "อูมามิ",
      tasteSweetness: "หวาน",
      tasteSourness: "เปรี้ยว",
      tasteSaltiness: "เค็ม",
      tasteBitterness: "ขม",
      tasteSpiciness: "เผ็ด",
      tasteRichness: "เข้มข้น",
      tasteLightness: "สดชื่น",
      tasteVolume: "ปริมาณ",
      tasteLocality: "ท้องถิ่น",
    },
    capture: {
      menuAnalysis: "Menu Analysis ({n} items)",
      imageAnalyzedWith: 'Analyzed image of "{q}".',
      imageAnalyzed: "Menu image analyzed.",
      notFound: "Restaurant not found",
      checkUrl: "Please check the URL",
      loading: "Loading...",
      writeReview: "Write a review",
      submitReview: "Write a review",
      reviewPlaceholder: "Food impressions, atmosphere, recommendations...",
      submitting: "Sending...",
      submit: "Submit",
      reviewSubmitted: "Review submitted!",
      comparing: "Comparing",
      compare: "Compare",
      less: "Less",
      moreDetails: "More details",
      relatedMenu: "Related",
      tellMeMore: "Tell me more about #{num} {name}",
      openMap: "Map",
    },
  },
  vi: {
    common: {
      back: "Quay lại",
      reload: "Tải lại",
      translate: "Dịch",
      openCamera: "Mở camera",
      menu: "Menu",
      guide: "Hướng dẫn",
    },
    language: {
      change: "Đổi ngôn ngữ",
      select: "Chọn ngôn ngữ",
      list: "Danh sách ngôn ngữ",
      modalTitle: "Ngôn ngữ",
      fallback: "Ngôn ngữ",
    },
    drawer: {
      historyLabel: "Ngăn lịch sử",
    },
    home: {
      bannerLabel: "Biểu ngữ",
      bannerAlt: "Biểu ngữ",
    },
    cta: {
      go: "Đi",
      continue: "Tiếp tục",
    },
    captureHeader: {
      menu: "Menu",
      scan: "Quét",
    },
    cameraPrompt: {
      openCamera: "Mở camera",
    },
    chat: {
      uploadPreview: "Xem trước tải lên",
      removeAttachment: "Xóa tệp đính kèm",
      tapToOpen: "Chạm để mở nhập liệu",
      camera: "Camera chat",
      gallery: "Thư viện chat",
      messageInput: "Nhập tin nhắn",
      sendMessage: "Gửi tin nhắn",
      send: "Gửi",
      sendDisabled: "Gửi bị tắt",
      attachmentGuide: "Hướng dẫn AI của bạn. Hỏi bất cứ điều gì.",
    },
    attachment: {
      cameraPhoto: "Ảnh từ camera",
      photoLibrary: "Thư viện ảnh",
    },
    camera: {
      unavailable: "Không khả dụng camera",
      close: "Đóng",
      translate: "Dịch",
      openGallery: "Mở thư viện",
      capturePhoto: "Chụp ảnh",
      flipCamera: "Chuyển camera",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "Quay lại",
      closeMenu: "Đóng menu",
      toggleMenu: "Chuyển menu",
      nav: {
        stores: "Cửa hàng",
        groups: "Nhóm",
        prompt: "Prompt",
        logout: "Đăng xuất",
      },
      title: {
        stores: "Cửa hàng",
      },
    },
    browser: {
      chromeMock: "Mô phỏng trình duyệt",
    },
    history: {
      sectionConversation: "Cuộc trò chuyện",
      newChat: "Trò chuyện mới",
      empty: "Chưa có cuộc trò chuyện nào",
      sectionExplore: "Khám phá thực đơn",
      favorites: "Yêu thích",
      popular: "Phổ biến",
      sectionStore: "Nhà hàng",
      storeInfo: "Thông tin nhà hàng",
      sectionPreferences: "Tùy chọn",
      preferencesAllergy: "Dị ứng & chế độ ăn",
      exploreOther: "Xem nhà hàng khác",
      title: "Tiêu đề lịch sử...",
    },
    hero: {
      heading: "Chụp và dịch",
      sub: "Chạm camera để bắt đầu",
    },
    explore: {
      title: "Khám phá ",
      subtitle: "Khám phá cùng ",
      placeholder: "Hỏi về khu vực...",
      searchPlaceholder: "Search restaurants",
      allCities: "All",
      nearbyTitle: "Nearby",
      listTitle: "Restaurants",
      loadMore: "Load more",
      noResults: "No restaurants found",
    },
    suggestions: {
      guide: "Hướng dẫn AI của bạn. Hỏi bất cứ điều gì.",
      chips: ["Dùng thế nào?", "Làm được gì?", "Quy tắc bàn ăn"],
    },
    restaurant: {
      title: "Select Restaurant",
      subtitle: "Choose a restaurant to explore",
      selectPlaceholder: "Select a restaurant...",
      continue: "Continue",
      noSelection: "Please select a restaurant",
      chatPlaceholder: "Hãy hỏi bất cứ điều gì",
      chatGuide: "Hướng dẫn AI cho {name} — Hỏi tôi bất cứ điều gì về nhà hàng này!",
      signatureDish: "Món đặc biệt ở đây là gì?",
      aboutCuisine: "Hãy cho biết về ẩm thực {cuisine}",
      bestTime: "Có gì ngon giới thiệu không?",
      dietaryOptions: "Tôi có đồ không ăn được",
      undecided: "Tôi đang phân vân",
      hearty: "Ăn no",
      singleDish: "Một món",
      lightAppetizer: "Khai vị / Salad",
      seasonalMenu: "Menu theo mùa?",
      drinksMenu: "Đồ uống?",
      googleReview: "Đánh giá",
    },
    nfg: {
      ingredients: "Nguyên liệu",
      allergens: "Chất gây dị ứng",
      restrictions: "Hạn chế ăn uống",
      flavorProfile: "Hồ sơ hương vị",
      calories: "Calo ước tính",
      taxNote: "Thuế",
      vadBadge: "VAD Đã xác minh",
      aiBadge: "AI Ước tính",
      confidence: "Độ tin cậy",
      texture: "Kết cấu",
      pairing: "Kết hợp với",
      howToEat: "Cách ăn",
      story: "Câu chuyện",
      servingStyle: "Cách phục vụ",
      tasteChart: "Vị",
      kidFriendly: "Phù hợp trẻ em",
      notKidFriendly: "Có thể không phù hợp trẻ em",
      suggestEdit: "Đề xuất chỉnh sửa",
      abv: "ABV",
      tasteUmami: "Umami",
      tasteSweetness: "Ngọt",
      tasteSourness: "Chua",
      tasteSaltiness: "Mặn",
      tasteBitterness: "Đắng",
      tasteSpiciness: "Cay",
      tasteRichness: "Đậm đà",
      tasteLightness: "Tươi mát",
      tasteVolume: "Khối lượng",
      tasteLocality: "Địa phương",
    },
    capture: {
      menuAnalysis: "Menu Analysis ({n} items)",
      imageAnalyzedWith: 'Analyzed image of "{q}".',
      imageAnalyzed: "Menu image analyzed.",
      notFound: "Restaurant not found",
      checkUrl: "Please check the URL",
      loading: "Loading...",
      writeReview: "Write a review",
      submitReview: "Write a review",
      reviewPlaceholder: "Food impressions, atmosphere, recommendations...",
      submitting: "Sending...",
      submit: "Submit",
      reviewSubmitted: "Review submitted!",
      comparing: "Comparing",
      compare: "Compare",
      less: "Less",
      moreDetails: "More details",
      relatedMenu: "Related",
      tellMeMore: "Tell me more about #{num} {name}",
      openMap: "Map",
    },
  },
  id: {
    common: {
      back: "Kembali",
      reload: "Muat ulang",
      translate: "Terjemahkan",
      openCamera: "Buka kamera",
      menu: "Menu",
      guide: "Panduan",
    },
    language: {
      change: "Ubah bahasa",
      select: "Pilih bahasa",
      list: "Daftar bahasa",
      modalTitle: "Bahasa",
      fallback: "Bahasa",
    },
    drawer: {
      historyLabel: "Laci riwayat",
    },
    home: {
      bannerLabel: "Banner",
      bannerAlt: "Banner",
    },
    cta: {
      go: "Pergi",
      continue: "Lanjut",
    },
    captureHeader: {
      menu: "Menu",
      scan: "Pindai",
    },
    cameraPrompt: {
      openCamera: "Buka kamera",
    },
    chat: {
      uploadPreview: "Pratinjau unggahan",
      removeAttachment: "Hapus lampiran",
      tapToOpen: "Ketuk untuk membuka input",
      camera: "Kamera chat",
      gallery: "Galeri chat",
      messageInput: "Input pesan",
      sendMessage: "Kirim pesan",
      send: "Kirim",
      sendDisabled: "Pengiriman dinonaktifkan",
      attachmentGuide: "Panduan AI Anda. Tanyakan apa saja.",
    },
    attachment: {
      cameraPhoto: "Foto kamera",
      photoLibrary: "Perpustakaan foto",
    },
    camera: {
      unavailable: "Kamera tidak tersedia",
      close: "Tutup",
      translate: "Terjemahkan",
      openGallery: "Buka galeri",
      capturePhoto: "Ambil foto",
      flipCamera: "Ganti kamera",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "Kembali",
      closeMenu: "Tutup menu",
      toggleMenu: "Ganti menu",
      nav: {
        stores: "Toko",
        groups: "Grup",
        prompt: "Prompt",
        logout: "Keluar",
      },
      title: {
        stores: "Toko",
      },
    },
    browser: {
      chromeMock: "Mock browser",
    },
    history: {
      sectionConversation: "Percakapan",
      newChat: "Obrolan baru",
      empty: "Belum ada percakapan",
      sectionExplore: "Jelajahi menu",
      favorites: "Favorit",
      popular: "Populer",
      sectionStore: "Restoran",
      storeInfo: "Info restoran",
      sectionPreferences: "Preferensi",
      preferencesAllergy: "Alergi & pola makan",
      exploreOther: "Lihat restoran lain",
      title: "Judul riwayat...",
    },
    hero: {
      heading: "Jepret & terjemahkan",
      sub: "Ketuk kamera untuk mulai",
    },
    explore: {
      title: "Jelajahi ",
      subtitle: "Jelajahi bersama ",
      placeholder: "Tanyakan tentang sekitar...",
      searchPlaceholder: "Search restaurants",
      allCities: "All",
      nearbyTitle: "Nearby",
      listTitle: "Restaurants",
      loadMore: "Load more",
      noResults: "No restaurants found",
    },
    suggestions: {
      guide: "Panduan AI Anda. Tanyakan apa saja.",
      chips: ["Cara pakai?", "Bisa apa?", "Etiket makan"],
    },
    restaurant: {
      title: "Select Restaurant",
      subtitle: "Choose a restaurant to explore",
      selectPlaceholder: "Select a restaurant...",
      continue: "Continue",
      noSelection: "Please select a restaurant",
      chatPlaceholder: "Tanya apa saja",
      chatGuide: "Panduan AI untuk {name} — Tanyakan apa saja tentang restoran ini!",
      signatureDish: "Apa menu andalannya?",
      aboutCuisine: "Ceritakan tentang masakan {cuisine}",
      bestTime: "Ada rekomendasi?",
      dietaryOptions: "Ada makanan yang nggak bisa saya makan",
      undecided: "Saya bingung pilih",
      hearty: "Yang mengenyangkan",
      singleDish: "Satu porsi",
      lightAppetizer: "Pembuka / Salad",
      seasonalMenu: "Menu musiman?",
      drinksMenu: "Minuman?",
      googleReview: "Ulasan",
    },
    nfg: {
      ingredients: "Bahan-bahan",
      allergens: "Alergen",
      restrictions: "Pembatasan Diet",
      flavorProfile: "Profil Rasa",
      calories: "Perkiraan Kalori",
      taxNote: "Pajak",
      vadBadge: "VAD Terverifikasi",
      aiBadge: "Perkiraan AI",
      confidence: "Kepercayaan",
      texture: "Tekstur",
      pairing: "Cocok dengan",
      howToEat: "Cara makan",
      story: "Cerita",
      servingStyle: "Penyajian",
      tasteChart: "Rasa",
      kidFriendly: "Ramah anak",
      notKidFriendly: "Mungkin tidak cocok untuk anak",
      suggestEdit: "Sarankan perubahan",
      abv: "ABV",
      tasteUmami: "Umami",
      tasteSweetness: "Manis",
      tasteSourness: "Asam",
      tasteSaltiness: "Asin",
      tasteBitterness: "Pahit",
      tasteSpiciness: "Pedas",
      tasteRichness: "Kaya rasa",
      tasteLightness: "Segar",
      tasteVolume: "Volume",
      tasteLocality: "Lokal",
    },
    capture: {
      menuAnalysis: "Menu Analysis ({n} items)",
      imageAnalyzedWith: 'Analyzed image of "{q}".',
      imageAnalyzed: "Menu image analyzed.",
      notFound: "Restaurant not found",
      checkUrl: "Please check the URL",
      loading: "Loading...",
      writeReview: "Write a review",
      submitReview: "Write a review",
      reviewPlaceholder: "Food impressions, atmosphere, recommendations...",
      submitting: "Sending...",
      submit: "Submit",
      reviewSubmitted: "Review submitted!",
      comparing: "Comparing",
      compare: "Compare",
      less: "Less",
      moreDetails: "More details",
      relatedMenu: "Related",
      tellMeMore: "Tell me more about #{num} {name}",
      openMap: "Map",
    },
  },
  ms: {
    common: {
      back: "Kembali",
      reload: "Muat semula",
      translate: "Terjemah",
      openCamera: "Buka kamera",
      menu: "Menu",
      guide: "Panduan",
    },
    language: {
      change: "Tukar bahasa",
      select: "Pilih bahasa",
      list: "Senarai bahasa",
      modalTitle: "Bahasa",
      fallback: "Bahasa",
    },
    drawer: {
      historyLabel: "Laci sejarah",
    },
    home: {
      bannerLabel: "Banner",
      bannerAlt: "Banner",
    },
    cta: {
      go: "Pergi",
      continue: "Teruskan",
    },
    captureHeader: {
      menu: "Menu",
      scan: "Imbas",
    },
    cameraPrompt: {
      openCamera: "Buka kamera",
    },
    chat: {
      uploadPreview: "Pratonton muat naik",
      removeAttachment: "Buang lampiran",
      tapToOpen: "Ketik untuk buka input",
      camera: "Kamera sembang",
      gallery: "Galeri sembang",
      messageInput: "Input mesej",
      sendMessage: "Hantar mesej",
      send: "Hantar",
      sendDisabled: "Hantaran dinyahaktifkan",
      attachmentGuide: "Panduan AI anda. Tanya apa sahaja.",
    },
    attachment: {
      cameraPhoto: "Foto kamera",
      photoLibrary: "Perpustakaan foto",
    },
    camera: {
      unavailable: "Kamera tidak tersedia",
      close: "Tutup",
      translate: "Terjemah",
      openGallery: "Buka galeri",
      capturePhoto: "Ambil foto",
      flipCamera: "Tukar kamera",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "Kembali",
      closeMenu: "Tutup menu",
      toggleMenu: "Tukar menu",
      nav: {
        stores: "Kedai",
        groups: "Kumpulan",
        prompt: "Prompt",
        logout: "Log keluar",
      },
      title: {
        stores: "Kedai",
      },
    },
    browser: {
      chromeMock: "Mock pelayar",
    },
    history: {
      sectionConversation: "Perbualan",
      newChat: "Sembang baharu",
      empty: "Belum ada perbualan",
      sectionExplore: "Terokai menu",
      favorites: "Kegemaran",
      popular: "Popular",
      sectionStore: "Restoran",
      storeInfo: "Maklumat restoran",
      sectionPreferences: "Keutamaan",
      preferencesAllergy: "Alahan & diet",
      exploreOther: "Lihat restoran lain",
      title: "Tajuk sejarah...",
    },
    hero: {
      heading: "Ambil & terjemah",
      sub: "Ketik kamera untuk mula",
    },
    explore: {
      title: "Terokai ",
      subtitle: "Terokai bersama ",
      placeholder: "Tanya tentang kawasan...",
      searchPlaceholder: "Search restaurants",
      allCities: "All",
      nearbyTitle: "Nearby",
      listTitle: "Restaurants",
      loadMore: "Load more",
      noResults: "No restaurants found",
    },
    suggestions: {
      guide: "Panduan AI anda. Tanya apa sahaja.",
      chips: ["Cara guna?", "Apa boleh buat?", "Etiket makan"],
    },
    restaurant: {
      title: "Select Restaurant",
      subtitle: "Choose a restaurant to explore",
      selectPlaceholder: "Select a restaurant...",
      continue: "Continue",
      noSelection: "Please select a restaurant",
      chatPlaceholder: "Tanya apa saja",
      chatGuide: "Panduan AI untuk {name} — Tanya apa sahaja tentang restoran ini!",
      signatureDish: "Apa yang special sini?",
      aboutCuisine: "Beritahu saya tentang masakan {cuisine}",
      bestTime: "Ada cadangan?",
      dietaryOptions: "Saya ada pantang makanan",
      undecided: "Saya tak pasti",
      hearty: "Yang mengenyangkan",
      singleDish: "Satu hidangan",
      lightAppetizer: "Pembuka / Salad",
      seasonalMenu: "Menu bermusim?",
      drinksMenu: "Minuman?",
      googleReview: "Ulasan",
    },
    nfg: {
      ingredients: "Bahan",
      allergens: "Alergen",
      restrictions: "Sekatan Pemakanan",
      flavorProfile: "Profil Rasa",
      calories: "Anggaran Kalori",
      taxNote: "Cukai",
      vadBadge: "VAD Disahkan",
      aiBadge: "Anggaran AI",
      confidence: "Keyakinan",
      texture: "Tekstur",
      pairing: "Sesuai dengan",
      howToEat: "Cara makan",
      story: "Kisah",
      servingStyle: "Hidangan",
      tasteChart: "Rasa",
      kidFriendly: "Mesra kanak-kanak",
      notKidFriendly: "Mungkin tidak sesuai untuk kanak-kanak",
      suggestEdit: "Cadangan pindaan",
      abv: "ABV",
      tasteUmami: "Umami",
      tasteSweetness: "Manis",
      tasteSourness: "Masam",
      tasteSaltiness: "Masin",
      tasteBitterness: "Pahit",
      tasteSpiciness: "Pedas",
      tasteRichness: "Kaya",
      tasteLightness: "Segar",
      tasteVolume: "Isipadu",
      tasteLocality: "Tempatan",
    },
    capture: {
      menuAnalysis: "Menu Analysis ({n} items)",
      imageAnalyzedWith: 'Analyzed image of "{q}".',
      imageAnalyzed: "Menu image analyzed.",
      notFound: "Restaurant not found",
      checkUrl: "Please check the URL",
      loading: "Loading...",
      writeReview: "Write a review",
      submitReview: "Write a review",
      reviewPlaceholder: "Food impressions, atmosphere, recommendations...",
      submitting: "Sending...",
      submit: "Submit",
      reviewSubmitted: "Review submitted!",
      comparing: "Comparing",
      compare: "Compare",
      less: "Less",
      moreDetails: "More details",
      relatedMenu: "Related",
      tellMeMore: "Tell me more about #{num} {name}",
      openMap: "Map",
    },
  },
  ar: {
    common: {
      back: "رجوع",
      reload: "إعادة تحميل",
      translate: "ترجمة",
      openCamera: "فتح الكاميرا",
      menu: "قائمة",
      guide: "دليل",
    },
    language: {
      change: "تغيير اللغة",
      select: "اختيار اللغة",
      list: "قائمة اللغات",
      modalTitle: "اللغة",
      fallback: "اللغة",
    },
    drawer: {
      historyLabel: "درج السجل",
    },
    home: {
      bannerLabel: "لافتة",
      bannerAlt: "لافتة",
    },
    cta: {
      go: "اذهب",
      continue: "متابعة",
    },
    captureHeader: {
      menu: "قائمة",
      scan: "مسح",
    },
    cameraPrompt: {
      openCamera: "فتح الكاميرا",
    },
    chat: {
      uploadPreview: "معاينة الرفع",
      removeAttachment: "إزالة المرفق",
      tapToOpen: "اضغط لفتح الإدخال",
      camera: "كاميرا الدردشة",
      gallery: "معرض الدردشة",
      messageInput: "إدخال الرسالة",
      sendMessage: "إرسال الرسالة",
      send: "إرسال",
      sendDisabled: "الإرسال معطل",
      attachmentGuide: "دليلك الذكي. اسأل أي شيء.",
    },
    attachment: {
      cameraPhoto: "صورة الكاميرا",
      photoLibrary: "مكتبة الصور",
    },
    camera: {
      unavailable: "الكاميرا غير متاحة",
      close: "إغلاق",
      translate: "ترجمة",
      openGallery: "فتح المعرض",
      capturePhoto: "التقاط صورة",
      flipCamera: "تبديل الكاميرا",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "رجوع",
      closeMenu: "إغلاق القائمة",
      toggleMenu: "تبديل القائمة",
      nav: {
        stores: "المتاجر",
        groups: "المجموعات",
        prompt: "Prompt",
        logout: "تسجيل الخروج",
      },
      title: {
        stores: "المتاجر",
      },
    },
    browser: {
      chromeMock: "نموذج واجهة المتصفح",
    },
    history: {
      sectionConversation: "المحادثات",
      newChat: "محادثة جديدة",
      empty: "لا توجد محادثات بعد",
      sectionExplore: "استكشاف القائمة",
      favorites: "المفضلة",
      popular: "الأكثر شهرة",
      sectionStore: "المطعم",
      storeInfo: "معلومات المطعم",
      sectionPreferences: "التفضيلات",
      preferencesAllergy: "الحساسية والنظام الغذائي",
      exploreOther: "استكشاف مطاعم أخرى",
      title: "عنوان السجل...",
    },
    hero: {
      heading: "التقط وترجم",
      sub: "اضغط على الكاميرا للبدء",
    },
    explore: {
      title: "استكشف ",
      subtitle: "استكشف مع ",
      placeholder: "اسأل عن المنطقة...",
      searchPlaceholder: "Search restaurants",
      allCities: "All",
      nearbyTitle: "Nearby",
      listTitle: "Restaurants",
      loadMore: "Load more",
      noResults: "No restaurants found",
    },
    suggestions: {
      guide: "دليلك الذكي. اسأل أي شيء.",
      chips: ["كيف أستخدمه؟", "ماذا يمكنه أن يفعل؟", "آداب الطعام"],
    },
    restaurant: {
      title: "Select Restaurant",
      subtitle: "Choose a restaurant to explore",
      selectPlaceholder: "Select a restaurant...",
      continue: "Continue",
      noSelection: "Please select a restaurant",
      chatPlaceholder: "اسألني أي شيء",
      chatGuide: "دليل الذكاء الاصطناعي لـ {name} — اسألني أي شيء عن هذا المطعم!",
      signatureDish: "شو الأكلة المميزة؟",
      aboutCuisine: "أخبرني عن مطبخ {cuisine}",
      bestTime: "شو بتنصحني؟",
      dietaryOptions: "عندي أكل ما بقدر آكله",
      undecided: "محتار",
      hearty: "وجبة دسمة",
      singleDish: "طبق واحد",
      lightAppetizer: "مقبلات / سلطة",
      seasonalMenu: "قائمة موسمية؟",
      drinksMenu: "مشروبات؟",
      googleReview: "تقييم",
    },
    nfg: {
      ingredients: "المكونات",
      allergens: "مسببات الحساسية",
      restrictions: "القيود الغذائية",
      flavorProfile: "ملف النكهة",
      calories: "السعرات التقديرية",
      taxNote: "الضريبة",
      vadBadge: "تم التحقق VAD",
      aiBadge: "تقدير AI",
      confidence: "الموثوقية",
      texture: "القوام",
      pairing: "يتناسب مع",
      howToEat: "طريقة الأكل",
      story: "القصة",
      servingStyle: "التقديم",
      tasteChart: "الطعم",
      kidFriendly: "مناسب للأطفال",
      notKidFriendly: "قد لا يناسب الأطفال",
      suggestEdit: "اقتراح تعديل",
      abv: "ABV",
      tasteUmami: "أومامي",
      tasteSweetness: "حلو",
      tasteSourness: "حامض",
      tasteSaltiness: "مالح",
      tasteBitterness: "مرّ",
      tasteSpiciness: "حارّ",
      tasteRichness: "غني",
      tasteLightness: "منعش",
      tasteVolume: "الحجم",
      tasteLocality: "محلي",
    },
    capture: {
      menuAnalysis: "Menu Analysis ({n} items)",
      imageAnalyzedWith: 'Analyzed image of "{q}".',
      imageAnalyzed: "Menu image analyzed.",
      notFound: "Restaurant not found",
      checkUrl: "Please check the URL",
      loading: "Loading...",
      writeReview: "Write a review",
      submitReview: "Write a review",
      reviewPlaceholder: "Food impressions, atmosphere, recommendations...",
      submitting: "Sending...",
      submit: "Submit",
      reviewSubmitted: "Review submitted!",
      comparing: "Comparing",
      compare: "Compare",
      less: "Less",
      moreDetails: "More details",
      relatedMenu: "Related",
      tellMeMore: "Tell me more about #{num} {name}",
      openMap: "Map",
    },
  },
  hi: {
    common: {
      back: "वापस",
      reload: "रीलोड",
      translate: "अनुवाद",
      openCamera: "कैमरा खोलें",
      menu: "मेनू",
      guide: "गाइड",
    },
    language: {
      change: "भाषा बदलें",
      select: "भाषा चुनें",
      list: "भाषा सूची",
      modalTitle: "भाषा",
      fallback: "भाषा",
    },
    drawer: {
      historyLabel: "इतिहास दराज",
    },
    home: {
      bannerLabel: "बैनर",
      bannerAlt: "बैनर",
    },
    cta: {
      go: "जाएं",
      continue: "जारी रखें",
    },
    captureHeader: {
      menu: "मेनू",
      scan: "स्कैन",
    },
    cameraPrompt: {
      openCamera: "कैमरा खोलें",
    },
    chat: {
      uploadPreview: "अपलोड पूर्वावलोकन",
      removeAttachment: "अटैचमेंट हटाएं",
      tapToOpen: "इनपुट खोलने के लिए टैप करें",
      camera: "चैट कैमरा",
      gallery: "चैट गैलरी",
      messageInput: "संदेश इनपुट",
      sendMessage: "संदेश भेजें",
      send: "भेजें",
      sendDisabled: "भेजना अक्षम",
      attachmentGuide: "आपका AI गाइड। कुछ भी पूछें।",
    },
    attachment: {
      cameraPhoto: "कैमरा फोटो",
      photoLibrary: "फोटो लाइब्रेरी",
    },
    camera: {
      unavailable: "कैमरा उपलब्ध नहीं",
      close: "बंद करें",
      translate: "अनुवाद",
      openGallery: "गैलरी खोलें",
      capturePhoto: "फोटो लें",
      flipCamera: "कैमरा बदलें",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "वापस",
      closeMenu: "मेनू बंद करें",
      toggleMenu: "मेनू बदलें",
      nav: {
        stores: "स्टोर",
        groups: "समूह",
        prompt: "प्रॉम्प्ट",
        logout: "लॉगआउट",
      },
      title: {
        stores: "स्टोर",
      },
    },
    browser: {
      chromeMock: "ब्राउज़र UI मॉक",
    },
    history: {
      sectionConversation: "बातचीत",
      newChat: "नई चैट",
      empty: "अभी तक कोई बातचीत नहीं",
      sectionExplore: "मेन्यू देखें",
      favorites: "पसंदीदा",
      popular: "लोकप्रिय",
      sectionStore: "रेस्तरां",
      storeInfo: "रेस्तरां जानकारी",
      sectionPreferences: "प्राथमिकताएं",
      preferencesAllergy: "एलर्जी और आहार",
      exploreOther: "अन्य रेस्तरां देखें",
      title: "इतिहास शीर्षक...",
    },
    hero: {
      heading: "फोटो लें और अनुवाद करें",
      sub: "शुरू करने के लिए कैमरा टैप करें",
    },
    explore: {
      title: " खोजें",
      subtitle: " के साथ खोजें",
      placeholder: "आसपास के बारे में पूछें...",
      searchPlaceholder: "Search restaurants",
      allCities: "All",
      nearbyTitle: "Nearby",
      listTitle: "Restaurants",
      loadMore: "Load more",
      noResults: "No restaurants found",
    },
    suggestions: {
      guide: "आपका AI गाइड। कुछ भी पूछें।",
      chips: ["कैसे उपयोग करें?", "क्या कर सकता है?", "भोजन शिष्टाचार"],
    },
    restaurant: {
      title: "Select Restaurant",
      subtitle: "Choose a restaurant to explore",
      selectPlaceholder: "Select a restaurant...",
      continue: "Continue",
      noSelection: "Please select a restaurant",
      chatPlaceholder: "कुछ भी पूछें",
      chatGuide: "{name} के लिए AI गाइड — इस रेस्तरां के बारे में कुछ भी पूछें!",
      signatureDish: "यहाँ की खास चीज़ क्या है?",
      aboutCuisine: "{cuisine} व्यंजन के बारे में बताएं",
      bestTime: "क्या recommend करोगे?",
      dietaryOptions: "मुझे कुछ चीज़ें नहीं खा सकता",
      undecided: "समझ नहीं आ रहा",
      hearty: "भरपेट",
      singleDish: "एक डिश",
      lightAppetizer: "स्टार्टर / सलाद",
      seasonalMenu: "सीज़नल मेनू?",
      drinksMenu: "ड्रिंक्स?",
      googleReview: "समीक्षा",
    },
    nfg: {
      ingredients: "सामग्री",
      allergens: "एलर्जेन",
      restrictions: "आहार प्रतिबंध",
      flavorProfile: "स्वाद प्रोफ़ाइल",
      calories: "अनुमानित कैलोरी",
      taxNote: "कर",
      vadBadge: "VAD सत्यापित",
      aiBadge: "AI अनुमान",
      confidence: "विश्वसनीयता",
      texture: "बनावट",
      pairing: "साथ में",
      howToEat: "कैसे खाएं",
      story: "कहानी",
      servingStyle: "परोसने का तरीका",
      tasteChart: "स्वाद",
      kidFriendly: "बच्चों के लिए उपयुक्त",
      notKidFriendly: "बच्चों के लिए उपयुक्त नहीं हो सकता",
      suggestEdit: "संशोधन सुझाएं",
      abv: "ABV",
      tasteUmami: "उमामी",
      tasteSweetness: "मीठा",
      tasteSourness: "खट्टा",
      tasteSaltiness: "नमकीन",
      tasteBitterness: "कड़वा",
      tasteSpiciness: "तीखा",
      tasteRichness: "समृद्ध",
      tasteLightness: "ताज़ा",
      tasteVolume: "मात्रा",
      tasteLocality: "स्थानीय",
    },
    capture: {
      menuAnalysis: "Menu Analysis ({n} items)",
      imageAnalyzedWith: 'Analyzed image of "{q}".',
      imageAnalyzed: "Menu image analyzed.",
      notFound: "Restaurant not found",
      checkUrl: "Please check the URL",
      loading: "Loading...",
      writeReview: "Write a review",
      submitReview: "Write a review",
      reviewPlaceholder: "Food impressions, atmosphere, recommendations...",
      submitting: "Sending...",
      submit: "Submit",
      reviewSubmitted: "Review submitted!",
      comparing: "Comparing",
      compare: "Compare",
      less: "Less",
      moreDetails: "More details",
      relatedMenu: "Related",
      tellMeMore: "Tell me more about #{num} {name}",
      openMap: "Map",
    },
  },
  tr: {
    common: {
      back: "Geri",
      reload: "Yenile",
      translate: "Çevir",
      openCamera: "Kamerayı aç",
      menu: "Menü",
      guide: "Rehber",
    },
    language: {
      change: "Dili değiştir",
      select: "Dil seç",
      list: "Dil listesi",
      modalTitle: "Dil",
      fallback: "Dil",
    },
    drawer: {
      historyLabel: "Geçmiş çekmecesi",
    },
    home: {
      bannerLabel: "Banner",
      bannerAlt: "Banner",
    },
    cta: {
      go: "Git",
      continue: "Devam",
    },
    captureHeader: {
      menu: "Menü",
      scan: "Tara",
    },
    cameraPrompt: {
      openCamera: "Kamerayı aç",
    },
    chat: {
      uploadPreview: "Yükleme önizlemesi",
      removeAttachment: "Eki kaldır",
      tapToOpen: "Girişi açmak için dokun",
      camera: "Sohbet kamerası",
      gallery: "Sohbet galerisi",
      messageInput: "Mesaj girişi",
      sendMessage: "Mesaj gönder",
      send: "Gönder",
      sendDisabled: "Gönderme devre dışı",
      attachmentGuide: "AI rehberiniz. Her şeyi sorabilirsiniz.",
    },
    attachment: {
      cameraPhoto: "Kamera fotoğrafı",
      photoLibrary: "Fotoğraf arşivi",
    },
    camera: {
      unavailable: "Kamera kullanılamıyor",
      close: "Kapat",
      translate: "Çevir",
      openGallery: "Galeriyi aç",
      capturePhoto: "Fotoğraf çek",
      flipCamera: "Kamerayı değiştir",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "Geri",
      closeMenu: "Menüyü kapat",
      toggleMenu: "Menüyü değiştir",
      nav: {
        stores: "Mağazalar",
        groups: "Gruplar",
        prompt: "Prompt",
        logout: "Çıkış yap",
      },
      title: {
        stores: "Mağazalar",
      },
    },
    browser: {
      chromeMock: "Tarayıcı arayüzü maketi",
    },
    history: {
      sectionConversation: "Sohbetler",
      newChat: "Yeni sohbet",
      empty: "Henüz sohbet yok",
      sectionExplore: "Menüyü keşfet",
      favorites: "Favoriler",
      popular: "Popüler",
      sectionStore: "Restoran",
      storeInfo: "Restoran bilgisi",
      sectionPreferences: "Tercihler",
      preferencesAllergy: "Alerjiler ve diyet",
      exploreOther: "Diğer restoranları gör",
      title: "Geçmiş başlığı...",
    },
    hero: {
      heading: "Çek ve çevir",
      sub: "Başlamak için kameraya dokun",
    },
    explore: {
      title: "'yu keşfet",
      subtitle: " ile keşfet",
      placeholder: "Bölge hakkında sor...",
      searchPlaceholder: "Search restaurants",
      allCities: "All",
      nearbyTitle: "Nearby",
      listTitle: "Restaurants",
      loadMore: "Load more",
      noResults: "No restaurants found",
    },
    suggestions: {
      guide: "AI rehberiniz. Her şeyi sorabilirsiniz.",
      chips: ["Nasıl kullanılır?", "Ne yapabilir?", "Yemek adabı"],
    },
    restaurant: {
      title: "Select Restaurant",
      subtitle: "Choose a restaurant to explore",
      selectPlaceholder: "Select a restaurant...",
      continue: "Continue",
      noSelection: "Please select a restaurant",
      chatPlaceholder: "Bana her şeyi sorabilirsiniz",
      chatGuide: "{name} için AI rehberi — Bu restoran hakkında bana her şeyi sorun!",
      signatureDish: "Buranın özel yemeği ne?",
      aboutCuisine: "{cuisine} mutfağı hakkında bilgi ver",
      bestTime: "Ne önerirsiniz?",
      dietaryOptions: "Yiyemediğim şeyler var",
      undecided: "Karar veremiyorum",
      hearty: "Doyurucu",
      singleDish: "Tek tabak",
      lightAppetizer: "Meze / Salata",
      seasonalMenu: "Mevsim menüsü?",
      drinksMenu: "İçecekler?",
      googleReview: "Yorum",
    },
    nfg: {
      ingredients: "Malzemeler",
      allergens: "Alerjenler",
      restrictions: "Diyet K\u0131s\u0131tlamalar\u0131",
      flavorProfile: "Lezzet Profili",
      calories: "Tahmini Kalori",
      taxNote: "Vergi",
      vadBadge: "VAD Do\u011Frulanm\u0131\u015F",
      aiBadge: "AI Tahmini",
      confidence: "G\u00FCvenilirlik",
      texture: "Doku",
      pairing: "\u0130le uyumlu",
      howToEat: "Nas\u0131l yenir",
      story: "Hikaye",
      servingStyle: "Servis",
      tasteChart: "Tat",
      kidFriendly: "\u00C7ocuklar i\u00E7in uygun",
      notKidFriendly: "\u00C7ocuklar i\u00E7in uygun olmayabilir",
      suggestEdit: "Düzenleme öner",
      abv: "ABV",
      tasteUmami: "Umami",
      tasteSweetness: "Tatlı",
      tasteSourness: "Ekşi",
      tasteSaltiness: "Tuzlu",
      tasteBitterness: "Acı",
      tasteSpiciness: "Baharatlı",
      tasteRichness: "Zengin",
      tasteLightness: "Taze",
      tasteVolume: "Hacim",
      tasteLocality: "Yerel",
    },
    capture: {
      menuAnalysis: "Menu Analysis ({n} items)",
      imageAnalyzedWith: 'Analyzed image of "{q}".',
      imageAnalyzed: "Menu image analyzed.",
      notFound: "Restaurant not found",
      checkUrl: "Please check the URL",
      loading: "Loading...",
      writeReview: "Write a review",
      submitReview: "Write a review",
      reviewPlaceholder: "Food impressions, atmosphere, recommendations...",
      submitting: "Sending...",
      submit: "Submit",
      reviewSubmitted: "Review submitted!",
      comparing: "Comparing",
      compare: "Compare",
      less: "Less",
      moreDetails: "More details",
      relatedMenu: "Related",
      tellMeMore: "Tell me more about #{num} {name}",
      openMap: "Map",
    },
  },
  bn: {
    common: {
      back: "ফিরে",
      reload: "রিলোড",
      translate: "অনুবাদ",
      openCamera: "ক্যামেরা খুলুন",
      menu: "মেনু",
      guide: "গাইড",
    },
    language: {
      change: "ভাষা পরিবর্তন",
      select: "ভাষা নির্বাচন",
      list: "ভাষার তালিকা",
      modalTitle: "ভাষা",
      fallback: "ভাষা",
    },
    drawer: {
      historyLabel: "ইতিহাস ড্রয়ার",
    },
    home: {
      bannerLabel: "ব্যানার",
      bannerAlt: "ব্যানার",
    },
    cta: {
      go: "যান",
      continue: "চালিয়ে যান",
    },
    captureHeader: {
      menu: "মেনু",
      scan: "স্ক্যান",
    },
    cameraPrompt: {
      openCamera: "ক্যামেরা খুলুন",
    },
    chat: {
      uploadPreview: "আপলোড প্রিভিউ",
      removeAttachment: "সংযুক্তি মুছুন",
      tapToOpen: "ইনপুট খুলতে ট্যাপ করুন",
      camera: "চ্যাট ক্যামেরা",
      gallery: "চ্যাট গ্যালারি",
      messageInput: "বার্তা ইনপুট",
      sendMessage: "বার্তা পাঠান",
      send: "পাঠান",
      sendDisabled: "পাঠানো নিষ্ক্রিয়",
      attachmentGuide: "আপনার AI গাইড। যেকোনো কিছু জিজ্ঞেস করুন।",
    },
    attachment: {
      cameraPhoto: "ক্যামেরা ছবি",
      photoLibrary: "ফটো লাইব্রেরি",
    },
    camera: {
      unavailable: "ক্যামেরা পাওয়া যাচ্ছে না",
      close: "বন্ধ করুন",
      translate: "অনুবাদ",
      openGallery: "গ্যালারি খুলুন",
      capturePhoto: "ছবি তুলুন",
      flipCamera: "ক্যামেরা বদলান",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "ফিরে",
      closeMenu: "মেনু বন্ধ করুন",
      toggleMenu: "মেনু বদলান",
      nav: {
        stores: "দোকান",
        groups: "গ্রুপ",
        prompt: "প্রম্পট",
        logout: "লগ আউট",
      },
      title: {
        stores: "দোকান",
      },
    },
    browser: {
      chromeMock: "ব্রাউজার UI মক",
    },
    history: {
      sectionConversation: "কথোপকথন",
      newChat: "নতুন চ্যাট",
      empty: "এখনও কোনো কথোপকথন নেই",
      sectionExplore: "মেনু দেখুন",
      favorites: "প্রিয়",
      popular: "জনপ্রিয়",
      sectionStore: "রেস্তোরাঁ",
      storeInfo: "রেস্তোরাঁর তথ্য",
      sectionPreferences: "পছন্দসমূহ",
      preferencesAllergy: "অ্যালার্জি ও খাদ্যাভ্যাস",
      exploreOther: "অন্যান্য রেস্তোরাঁ দেখুন",
      title: "ইতিহাস শিরোনাম...",
    },
    hero: {
      heading: "ছবি তুলে অনুবাদ",
      sub: "শুরু করতে ক্যামেরা ট্যাপ করুন",
    },
    explore: {
      title: " অন্বেষণ করুন",
      subtitle: " এর সাথে অন্বেষণ",
      placeholder: "আশেপাশে সম্পর্কে জিজ্ঞেস করুন...",
      searchPlaceholder: "Search restaurants",
      allCities: "All",
      nearbyTitle: "Nearby",
      listTitle: "Restaurants",
      loadMore: "Load more",
      noResults: "No restaurants found",
    },
    suggestions: {
      guide: "আপনার AI গাইড। যেকোনো কিছু জিজ্ঞেস করুন।",
      chips: ["কীভাবে ব্যবহার করব?", "কী করতে পারে?", "খাবার শিষ্টাচার"],
    },
    restaurant: {
      title: "Select Restaurant",
      subtitle: "Choose a restaurant to explore",
      selectPlaceholder: "Select a restaurant...",
      continue: "Continue",
      noSelection: "Please select a restaurant",
      chatPlaceholder: "যেকোনো কিছু জিজ্ঞাসা করুন",
      chatGuide: "{name} এর জন্য AI গাইড — এই রেস্তোরাঁ সম্পর্কে যে কোনো কিছু জিজ্ঞাসা করুন!",
      signatureDish: "এখানকার স্পেশাল কী?",
      aboutCuisine: "{cuisine} রন্ধনপ্রণালী সম্পর্কে বলুন",
      bestTime: "কী খেতে ভালো?",
      dietaryOptions: "আমি কিছু জিনিস খেতে পারি না",
      undecided: "ঠিক করতে পারছি না",
      hearty: "ভরপেট",
      singleDish: "একটা ডিশ",
      lightAppetizer: "স্টার্টার / সালাদ",
      seasonalMenu: "সিজনাল মেনু?",
      drinksMenu: "পানীয়?",
      googleReview: "রিভিউ",
    },
    nfg: {
      ingredients: "উপকরণ",
      allergens: "অ্যালার্জেন",
      restrictions: "খাদ্য বিধিনিষেধ",
      flavorProfile: "স্বাদের বৈশিষ্ট্য",
      calories: "আনুমানিক ক্যালোরি",
      taxNote: "কর",
      vadBadge: "VAD যাচাইকৃত",
      aiBadge: "AI অনুমান",
      confidence: "নির্ভরযোগ্যতা",
      texture: "গঠন",
      pairing: "সাথে মানানসই",
      howToEat: "কীভাবে খাবেন",
      story: "গল্প",
      servingStyle: "পরিবেশন",
      tasteChart: "স্বাদ",
      kidFriendly: "শিশুদের জন্য উপযোগী",
      notKidFriendly: "শিশুদের জন্য উপযুক্ত নাও হতে পারে",
      suggestEdit: "সম্পাদনা প্রস্তাব",
      abv: "ABV",
      tasteUmami: "উমামি",
      tasteSweetness: "মিষ্টি",
      tasteSourness: "টক",
      tasteSaltiness: "নোনতা",
      tasteBitterness: "তিক্ত",
      tasteSpiciness: "ঝাল",
      tasteRichness: "সমৃদ্ধ",
      tasteLightness: "তাজা",
      tasteVolume: "পরিমাণ",
      tasteLocality: "স্থানীয়",
    },
    capture: {
      menuAnalysis: "Menu Analysis ({n} items)",
      imageAnalyzedWith: 'Analyzed image of "{q}".',
      imageAnalyzed: "Menu image analyzed.",
      notFound: "Restaurant not found",
      checkUrl: "Please check the URL",
      loading: "Loading...",
      writeReview: "Write a review",
      submitReview: "Write a review",
      reviewPlaceholder: "Food impressions, atmosphere, recommendations...",
      submitting: "Sending...",
      submit: "Submit",
      reviewSubmitted: "Review submitted!",
      comparing: "Comparing",
      compare: "Compare",
      less: "Less",
      moreDetails: "More details",
      relatedMenu: "Related",
      tellMeMore: "Tell me more about #{num} {name}",
      openMap: "Map",
    },
  },
  my: {
    common: {
      back: "နောက်သို့",
      reload: "ပြန်လည်ဖွင့်",
      translate: "ဘာသာပြန်",
      openCamera: "ကင်မရာဖွင့်",
      menu: "မီနူး",
      guide: "လမ်းညွှန်",
    },
    language: {
      change: "ဘာသာစကားပြောင်း",
      select: "ဘာသာစကားရွေး",
      list: "ဘာသာစကားစာရင်း",
      modalTitle: "ဘာသာစကား",
      fallback: "ဘာသာစကား",
    },
    drawer: {
      historyLabel: "မှတ်တမ်းဘူး",
    },
    home: {
      bannerLabel: "ဘန်နာ",
      bannerAlt: "ဘန်နာ",
    },
    cta: {
      go: "သွား",
      continue: "ဆက်လုပ်",
    },
    captureHeader: {
      menu: "မီနူး",
      scan: "စကန်",
    },
    cameraPrompt: {
      openCamera: "ကင်မရာဖွင့်",
    },
    chat: {
      uploadPreview: "တင်သွင်းမှုကြိုကြည့်",
      removeAttachment: "တွဲဖက်ဖိုင်ဖယ်ရှား",
      tapToOpen: "ထည့်သွင်းရန်နှိပ်ပါ",
      camera: "ချတ်ကင်မရာ",
      gallery: "ချတ်ဂယ်လာရီ",
      messageInput: "စာတိုထည့်သွင်း",
      sendMessage: "စာတိုပို့",
      send: "ပို့",
      sendDisabled: "ပို့ခြင်းမရ",
      attachmentGuide: "သင့် AI လမ်းညွှန်။ မေးချင်သမျှမေးပါ။",
    },
    attachment: {
      cameraPhoto: "ကင်မရာဓာတ်ပုံ",
      photoLibrary: "ဓာတ်ပုံစာကြည့်တိုက်",
    },
    camera: {
      unavailable: "ကင်မရာမရရှိနိုင်",
      close: "ပိတ်",
      translate: "ဘာသာပြန်",
      openGallery: "ဂယ်လာရီဖွင့်",
      capturePhoto: "ဓာတ်ပုံရိုက်",
      flipCamera: "ကင်မရာပြောင်း",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "နောက်သို့",
      closeMenu: "မီနူးပိတ်",
      toggleMenu: "မီနူးပြောင်း",
      nav: {
        stores: "စတိုးများ",
        groups: "အဖွဲ့များ",
        prompt: "Prompt",
        logout: "ထွက်ရန်",
      },
      title: {
        stores: "စတိုးများ",
      },
    },
    browser: {
      chromeMock: "ဘရောင်ဇာ UI မော့ခ်",
    },
    history: {
      sectionConversation: "စကားဝိုင်းများ",
      newChat: "ချတ်အသစ်",
      empty: "စကားဝိုင်း မရှိသေးပါ",
      sectionExplore: "မီနူးကို ရှာဖွေပါ",
      favorites: "အကြိုက်ဆုံးများ",
      popular: "ရေပန်းစားသော",
      sectionStore: "ဆိုင်",
      storeInfo: "ဆိုင် အချက်အလက်",
      sectionPreferences: "ဦးစားပေးချက်များ",
      preferencesAllergy: "ဓာတ်မတည့်မှု နှင့် အစားအသောက်",
      exploreOther: "အခြားဆိုင်များ ကြည့်ရန်",
      title: "မှတ်တမ်းခေါင်းစဉ်...",
    },
    hero: {
      heading: "ရိုက်ပြီးဘာသာပြန်",
      sub: "ကင်မရာကိုနှိပ်၍စတင်",
    },
    explore: {
      title: " ကိုလေ့လာ",
      subtitle: " နဲ့လေ့လာ",
      placeholder: "ပတ်ဝန်းကျင်မေးပါ...",
      searchPlaceholder: "Search restaurants",
      allCities: "All",
      nearbyTitle: "Nearby",
      listTitle: "Restaurants",
      loadMore: "Load more",
      noResults: "No restaurants found",
    },
    suggestions: {
      guide: "သင့် AI လမ်းညွှန်။ မေးချင်သမျှမေးပါ။",
      chips: ["ဘယ်လိုသုံးမလဲ?", "ဘာလုပ်နိုင်လဲ?", "စားသောက်မှုယဉ်ကျေးမှု"],
    },
    restaurant: {
      title: "Select Restaurant",
      subtitle: "Choose a restaurant to explore",
      selectPlaceholder: "Select a restaurant...",
      continue: "Continue",
      noSelection: "Please select a restaurant",
      chatPlaceholder: "ဘာမဆိုမေးပါ",
      chatGuide: "{name} အတွက် AI လမ်းညွှန် — ဤစားသောက်ဆိုင်အကြောင်းမေးချင်တာမေးပါ!",
      signatureDish: "ဒီဆိုင်ရဲ့အထူးက ဘာလဲ?",
      aboutCuisine: "{cuisine} အစားအစာအကြောင်း ပြောပြပါ",
      bestTime: "ဘာစားရင်ကောင်းမလဲ?",
      dietaryOptions: "မစားနိုင်တာတွေ ရှိတယ်",
      undecided: "ဆုံးဖြတ်မရသေးဘူး",
      hearty: "ဗိုက်ပြည့်အောင်",
      singleDish: "တစ်ပွဲ",
      lightAppetizer: "အရှေ့ဟင်း / သုပ်",
      seasonalMenu: "ရာသီမီနူး?",
      drinksMenu: "အချိုရည်?",
      googleReview: "သုံးသပ်ချက်",
    },
    nfg: {
      ingredients: "ပါဝင်ပစ္စည်းများ",
      allergens: "ဓာတ်မတည့်မှုရှိနိုင်သောအရာများ",
      restrictions: "အစားအသောက်ကန့်သတ်ချက်များ",
      flavorProfile: "အရသာပရိုဖိုင်",
      calories: "ခန့်မှန်းကယ်လိုရီ",
      taxNote: "အခွန်",
      vadBadge: "VAD အတည်ပြုပြီး",
      aiBadge: "AI ခန့်မှန်း",
      confidence: "ယုံကြည်မှု",
      texture: "အသွေးအရည်",
      pairing: "တွဲဖက်",
      howToEat: "စားနည်း",
      story: "ပုံပြင်",
      servingStyle: "ဧည့်ခံပုံ",
      tasteChart: "အရသာ",
      kidFriendly: "ကလေးများအတွက်သင့်တော်",
      notKidFriendly: "ကလေးများအတွက်မသင့်တော်နိုင်ပါ",
      suggestEdit: "ပြင်ဆင်ချက်အကြံပြု",
      abv: "ABV",
      tasteUmami: "အူမမီ",
      tasteSweetness: "ချို",
      tasteSourness: "ချဉ်",
      tasteSaltiness: "ငန်",
      tasteBitterness: "ခါး",
      tasteSpiciness: "စပ်",
      tasteRichness: "ထူထပ်",
      tasteLightness: "လတ်ဆတ်",
      tasteVolume: "ပမာဏ",
      tasteLocality: "ဒေသ",
    },
    capture: {
      menuAnalysis: "Menu Analysis ({n} items)",
      imageAnalyzedWith: 'Analyzed image of "{q}".',
      imageAnalyzed: "Menu image analyzed.",
      notFound: "Restaurant not found",
      checkUrl: "Please check the URL",
      loading: "Loading...",
      writeReview: "Write a review",
      submitReview: "Write a review",
      reviewPlaceholder: "Food impressions, atmosphere, recommendations...",
      submitting: "Sending...",
      submit: "Submit",
      reviewSubmitted: "Review submitted!",
      comparing: "Comparing",
      compare: "Compare",
      less: "Less",
      moreDetails: "More details",
      relatedMenu: "Related",
      tellMeMore: "Tell me more about #{num} {name}",
      openMap: "Map",
    },
  },
  tl: {
    common: {
      back: "Bumalik",
      reload: "I-reload",
      translate: "Isalin",
      openCamera: "Buksan ang camera",
      menu: "Menu",
      guide: "Gabay",
    },
    language: {
      change: "Palitan ang wika",
      select: "Pumili ng wika",
      list: "Listahan ng wika",
      modalTitle: "Wika",
      fallback: "Wika",
    },
    drawer: {
      historyLabel: "Drawer ng kasaysayan",
    },
    home: {
      bannerLabel: "Banner",
      bannerAlt: "Banner",
    },
    cta: {
      go: "Pumunta",
      continue: "Magpatuloy",
    },
    captureHeader: {
      menu: "Menu",
      scan: "I-scan",
    },
    cameraPrompt: {
      openCamera: "Buksan ang camera",
    },
    chat: {
      uploadPreview: "Preview ng upload",
      removeAttachment: "Alisin ang attachment",
      tapToOpen: "I-tap para buksan ang input",
      camera: "Camera ng chat",
      gallery: "Gallery ng chat",
      messageInput: "Input ng mensahe",
      sendMessage: "Ipadala ang mensahe",
      send: "Ipadala",
      sendDisabled: "Naka-disable ang padala",
      attachmentGuide: "Ang iyong AI guide. Magtanong ng kahit ano.",
    },
    attachment: {
      cameraPhoto: "Larawan ng camera",
      photoLibrary: "Photo library",
    },
    camera: {
      unavailable: "Hindi available ang camera",
      close: "Isara",
      translate: "Isalin",
      openGallery: "Buksan ang gallery",
      capturePhoto: "Kumuha ng larawan",
      flipCamera: "Palitan ang camera",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "Bumalik",
      closeMenu: "Isara ang menu",
      toggleMenu: "Palitan ang menu",
      nav: {
        stores: "Mga tindahan",
        groups: "Mga grupo",
        prompt: "Prompt",
        logout: "Mag-logout",
      },
      title: {
        stores: "Mga tindahan",
      },
    },
    browser: {
      chromeMock: "Mock ng browser",
    },
    history: {
      sectionConversation: "Mga usapan",
      newChat: "Bagong chat",
      empty: "Wala pang usapan",
      sectionExplore: "Tuklasin ang menu",
      favorites: "Mga paborito",
      popular: "Sikat",
      sectionStore: "Restawran",
      storeInfo: "Impormasyon ng restawran",
      sectionPreferences: "Mga kagustuhan",
      preferencesAllergy: "Allergy at diyeta",
      exploreOther: "Tingnan ang ibang restawran",
      title: "Pamagat ng kasaysayan...",
    },
    hero: {
      heading: "Kunan at isalin",
      sub: "I-tap ang camera para magsimula",
    },
    explore: {
      title: "Tuklasin ang ",
      subtitle: "Tuklasin kasama ang ",
      placeholder: "Magtanong tungkol sa lugar...",
      searchPlaceholder: "Search restaurants",
      allCities: "All",
      nearbyTitle: "Nearby",
      listTitle: "Restaurants",
      loadMore: "Load more",
      noResults: "No restaurants found",
    },
    suggestions: {
      guide: "Ang iyong AI guide. Magtanong ng kahit ano.",
      chips: ["Paano gamitin?", "Ano ang kaya?", "Etiquette sa pagkain"],
    },
    restaurant: {
      title: "Select Restaurant",
      subtitle: "Choose a restaurant to explore",
      selectPlaceholder: "Select a restaurant...",
      continue: "Continue",
      noSelection: "Please select a restaurant",
      chatPlaceholder: "Magtanong ng kahit ano",
      chatGuide: "Gabay na AI para sa {name} — Magtanong tungkol sa restaurant na ito!",
      signatureDish: "Ano ang espesyal dito?",
      aboutCuisine: "Sabihin sa akin tungkol sa lutuing {cuisine}",
      bestTime: "Ano ang masarap?",
      dietaryOptions: "May hindi ako makakain",
      undecided: "Hindi ko mapagpasyahan",
      hearty: "Busog na pagkain",
      singleDish: "Isang ulam",
      lightAppetizer: "Pampagana / Salad",
      seasonalMenu: "Seasonal menu?",
      drinksMenu: "Inumin?",
      googleReview: "Review",
    },
    nfg: {
      ingredients: "Sangkap",
      allergens: "Mga Allergen",
      restrictions: "Mga Paghihigpit sa Pagkain",
      flavorProfile: "Profile ng Lasa",
      calories: "Tinatayang Calorie",
      taxNote: "Buwis",
      vadBadge: "VAD Na-verify",
      aiBadge: "AI Tinantya",
      confidence: "Kumpiyansa",
      texture: "Texture",
      pairing: "Bagay sa",
      howToEat: "Paano kainin",
      story: "Kwento",
      servingStyle: "Paraan ng Paghahain",
      tasteChart: "Lasa",
      kidFriendly: "Para sa bata",
      notKidFriendly: "Maaaring hindi angkop sa bata",
      suggestEdit: "Magmungkahi ng pagbabago",
      abv: "ABV",
      tasteUmami: "Umami",
      tasteSweetness: "Matamis",
      tasteSourness: "Maasim",
      tasteSaltiness: "Maalat",
      tasteBitterness: "Mapait",
      tasteSpiciness: "Maanghang",
      tasteRichness: "Mayaman",
      tasteLightness: "Sariwà",
      tasteVolume: "Dami",
      tasteLocality: "Lokal",
    },
    capture: {
      menuAnalysis: "Menu Analysis ({n} items)",
      imageAnalyzedWith: 'Analyzed image of "{q}".',
      imageAnalyzed: "Menu image analyzed.",
      notFound: "Restaurant not found",
      checkUrl: "Please check the URL",
      loading: "Loading...",
      writeReview: "Write a review",
      submitReview: "Write a review",
      reviewPlaceholder: "Food impressions, atmosphere, recommendations...",
      submitting: "Sending...",
      submit: "Submit",
      reviewSubmitted: "Review submitted!",
      comparing: "Comparing",
      compare: "Compare",
      less: "Less",
      moreDetails: "More details",
      relatedMenu: "Related",
      tellMeMore: "Tell me more about #{num} {name}",
      openMap: "Map",
    },
  },
  lo: {
    common: {
      back: "ກັບ",
      reload: "ໂຫລດໃໝ່",
      translate: "ແປ",
      openCamera: "ເປີດກ້ອງ",
      menu: "ເມນູ",
      guide: "ຄູ່ມື",
    },
    language: {
      change: "ປ່ຽນພາສາ",
      select: "ເລືອກພາສາ",
      list: "ລາຍຊື່ພາສາ",
      modalTitle: "ພາສາ",
      fallback: "ພາສາ",
    },
    drawer: {
      historyLabel: "ລິ້ນຊັກປະຫວັດ",
    },
    home: {
      bannerLabel: "ແບນເນີ",
      bannerAlt: "ແບນເນີ",
    },
    cta: {
      go: "ໄປ",
      continue: "ດຳເນີນຕໍ່",
    },
    captureHeader: {
      menu: "ເມນູ",
      scan: "ສະແກນ",
    },
    cameraPrompt: {
      openCamera: "ເປີດກ້ອງ",
    },
    chat: {
      uploadPreview: "ຕົວຢ່າງອັບໂຫລດ",
      removeAttachment: "ລຶບໄຟລ໌ແນບ",
      tapToOpen: "ແຕະເພື່ອເປີດການປ້ອນ",
      camera: "ກ້ອງແຊັດ",
      gallery: "ແກລເລີແຊັດ",
      messageInput: "ການປ້ອນຂໍ້ຄວາມ",
      sendMessage: "ສົ່ງຂໍ້ຄວາມ",
      send: "ສົ່ງ",
      sendDisabled: "ສົ່ງບໍ່ໄດ້",
      attachmentGuide: "ຜູ້ນໍາທາງ AI ຂອງທ່ານ. ຖາມໄດ້ທຸກຢ່າງ.",
    },
    attachment: {
      cameraPhoto: "ຮູບກ້ອງ",
      photoLibrary: "ຄັງຮູບ",
    },
    camera: {
      unavailable: "ກ້ອງບໍ່ພ້ອມໃຊ້",
      close: "ປິດ",
      translate: "ແປ",
      openGallery: "ເປີດແກລເລີ",
      capturePhoto: "ຖ່າຍຮູບ",
      flipCamera: "ສະຫຼັບກ້ອງ",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "ກັບ",
      closeMenu: "ປິດເມນູ",
      toggleMenu: "ສະຫຼັບເມນູ",
      nav: {
        stores: "ຮ້ານ",
        groups: "ກຸ່ມ",
        prompt: "Prompt",
        logout: "ອອກຈາກລະບົບ",
      },
      title: {
        stores: "ຮ້ານ",
      },
    },
    browser: {
      chromeMock: "ຈຳລອງ UI ບຣາວເຊີ",
    },
    history: {
      sectionConversation: "ການສົນທະນາ",
      newChat: "ສົນທະນາໃໝ່",
      empty: "ຍັງບໍ່ມີການສົນທະນາ",
      sectionExplore: "ສຳຫຼວດເມນູ",
      favorites: "ລາຍການທີ່ມັກ",
      popular: "ນິຍົມ",
      sectionStore: "ຮ້ານ",
      storeInfo: "ຂໍ້ມູນຮ້ານ",
      sectionPreferences: "ການຕັ້ງຄ່າ",
      preferencesAllergy: "ອາການແພ້ ແລະ ການກິນ",
      exploreOther: "ເບິ່ງຮ້ານອື່ນ",
      title: "ຫົວຂໍ້ປະຫວັດ...",
    },
    hero: {
      heading: "ຖ່າຍແລະແປ",
      sub: "ແຕະກ້ອງເພື່ອເລີ່ມ",
    },
    explore: {
      title: "ສຳຫຼວດ ",
      subtitle: "ສຳຫຼວດກັບ ",
      placeholder: "ຖາມກ່ຽວກັບບໍລິເວນ...",
      searchPlaceholder: "Search restaurants",
      allCities: "All",
      nearbyTitle: "Nearby",
      listTitle: "Restaurants",
      loadMore: "Load more",
      noResults: "No restaurants found",
    },
    suggestions: {
      guide: "ຜູ້ນໍາທາງ AI ຂອງທ່ານ. ຖາມໄດ້ທຸກຢ່າງ.",
      chips: ["ໃຊ້ແນວໃດ?", "ທຳອະໄດ້ແນວໃດ?", "ມາລະຍາດທາງອາຫານ"],
    },
    restaurant: {
      title: "Select Restaurant",
      subtitle: "Choose a restaurant to explore",
      selectPlaceholder: "Select a restaurant...",
      continue: "Continue",
      noSelection: "Please select a restaurant",
      chatPlaceholder: "ຖາມຫຍັງກໍໄດ້",
      chatGuide: "ຄູ່ມື AI ສຳລັບ {name} — ຖາມຫຍັງກໍໄດ້ກ່ຽວກັບຮ້ານອາຫານນີ້!",
      signatureDish: "ອາຫານເດັ່ນທີ່ນີ້ແມ່ນຫຍັງ?",
      aboutCuisine: "ບອກກ່ຽວກັບອາຫານ {cuisine}",
      bestTime: "ແນະນຳອັນໃດແດ່?",
      dietaryOptions: "ມີອາຫານທີ່ຂ້ອຍກິນບໍ່ໄດ້",
      undecided: "ຍັງຕັດສິນໃຈບໍ່ໄດ້",
      hearty: "ອາຫານຈານໜັກ",
      singleDish: "ອາຫານຈານດຽວ",
      lightAppetizer: "ອາຫານເບົາ / ສະລັດ",
      seasonalMenu: "ເມນູຕາມລະດູ?",
      drinksMenu: "ເຄື່ອງດື່ມ?",
      googleReview: "ລີວິວ",
    },
    nfg: {
      ingredients: "ສ່ວນປະກອບ",
      allergens: "ສານກໍ່ພູມແພ້",
      restrictions: "ຂໍ້ຈຳກັດດ້ານອາຫານ",
      flavorProfile: "ລັກສະນະລົດຊາດ",
      calories: "ແຄລໍລີ່ປະມານ",
      taxNote: "ພາສີ",
      vadBadge: "VAD ຢືນຢັນແລ້ວ",
      aiBadge: "AI ປະເມີນ",
      confidence: "ຄວາມໜ້າເຊື່ອຖື",
      texture: "ເນື້ອສຳຜັດ",
      pairing: "ເຂົ້າກັນກັບ",
      howToEat: "ວິທີກິນ",
      story: "ເລື່ອງລາວ",
      servingStyle: "ການເສີບ",
      tasteChart: "ລົດຊາດ",
      kidFriendly: "ເໝາະສຳລັບເດັກ",
      notKidFriendly: "ອາດບໍ່ເໝາະສຳລັບເດັກ",
      suggestEdit: "ແນະນຳການແກ້ໄຂ",
      abv: "ABV",
      tasteUmami: "ອູມາມິ",
      tasteSweetness: "ຫວານ",
      tasteSourness: "ສົ້ມ",
      tasteSaltiness: "ເຄັມ",
      tasteBitterness: "ຂົມ",
      tasteSpiciness: "ເຜັດ",
      tasteRichness: "ເຂັ້ມ",
      tasteLightness: "ສົດຊື່ນ",
      tasteVolume: "ປະລິມານ",
      tasteLocality: "ທ້ອງຖິ່ນ",
    },
    capture: {
      menuAnalysis: "Menu Analysis ({n} items)",
      imageAnalyzedWith: 'Analyzed image of "{q}".',
      imageAnalyzed: "Menu image analyzed.",
      notFound: "Restaurant not found",
      checkUrl: "Please check the URL",
      loading: "Loading...",
      writeReview: "Write a review",
      submitReview: "Write a review",
      reviewPlaceholder: "Food impressions, atmosphere, recommendations...",
      submitting: "Sending...",
      submit: "Submit",
      reviewSubmitted: "Review submitted!",
      comparing: "Comparing",
      compare: "Compare",
      less: "Less",
      moreDetails: "More details",
      relatedMenu: "Related",
      tellMeMore: "Tell me more about #{num} {name}",
      openMap: "Map",
    },
  },
  km: {
    common: {
      back: "ថយក្រោយ",
      reload: "ផ្ទុកឡើងវិញ",
      translate: "បកប្រែ",
      openCamera: "បើកកាមេរ៉ា",
      menu: "ម៉ឺនុយ",
      guide: "មគ្គុទេសក៍",
    },
    language: {
      change: "ប្តូរភាសា",
      select: "ជ្រើសភាសា",
      list: "បញ្ជីភាសា",
      modalTitle: "ភាសា",
      fallback: "ភាសា",
    },
    drawer: {
      historyLabel: "ថតប្រវត្តិ",
    },
    home: {
      bannerLabel: "បដា",
      bannerAlt: "បដា",
    },
    cta: {
      go: "ទៅ",
      continue: "បន្ត",
    },
    captureHeader: {
      menu: "ម៉ឺនុយ",
      scan: "ស្កេន",
    },
    cameraPrompt: {
      openCamera: "បើកកាមេរ៉ា",
    },
    chat: {
      uploadPreview: "មើលជាមុនការផ្ទុក",
      removeAttachment: "លុបឯកសារភ្ជាប់",
      tapToOpen: "ប៉ះដើម្បីបើកបញ្ចូល",
      camera: "កាមេរ៉ាជជែក",
      gallery: "កែឡារីជជែក",
      messageInput: "បញ្ចូលសារ",
      sendMessage: "ផ្ញើសារ",
      send: "ផ្ញើ",
      sendDisabled: "ផ្ញើមិនអាច",
      attachmentGuide: "មគ្គុទេសក៍ AI របស់អ្នក។ សួរអ្វីក៏បាន។",
    },
    attachment: {
      cameraPhoto: "រូបថតកាមេរ៉ា",
      photoLibrary: "បណ្ណាល័យរូបថត",
    },
    camera: {
      unavailable: "កាមេរ៉ាមិនមាន",
      close: "បិទ",
      translate: "បកប្រែ",
      openGallery: "បើកកែឡារី",
      capturePhoto: "ថតរូប",
      flipCamera: "ប្តូរកាមេរ៉ា",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "ថយក្រោយ",
      closeMenu: "បិទម៉ឺនុយ",
      toggleMenu: "ប្តូរម៉ឺនុយ",
      nav: {
        stores: "ហាង",
        groups: "ក្រុម",
        prompt: "Prompt",
        logout: "ចាកចេញ",
      },
      title: {
        stores: "ហាង",
      },
    },
    browser: {
      chromeMock: "ម៉ុក UI កម្មវិធីរុករក",
    },
    history: {
      sectionConversation: "ការសន្ទនា",
      newChat: "ជជែកថ្មី",
      empty: "មិនទាន់មានការសន្ទនា",
      sectionExplore: "រុករកម៉ឺនុយ",
      favorites: "សំណព្វ",
      popular: "ពេញនិយម",
      sectionStore: "ភោជនីយដ្ឋាន",
      storeInfo: "ព័ត៌មានភោជនីយដ្ឋាន",
      sectionPreferences: "ចំណូលចិត្ត",
      preferencesAllergy: "ភាពអាឡែស៊ី និងរបបអាហារ",
      exploreOther: "មើលភោជនីយដ្ឋានផ្សេងទៀត",
      title: "ចំណងជើងប្រវត្តិ...",
    },
    hero: {
      heading: "ថតហើយបកប្រែ",
      sub: "ប៉ះកាមេរ៉ាដើម្បីចាប់ផ្តើម",
    },
    explore: {
      title: "ស្វែងយល់ ",
      subtitle: "ស្វែងយល់ជាមួយ ",
      placeholder: "សួរអំពីតំបន់...",
      searchPlaceholder: "Search restaurants",
      allCities: "All",
      nearbyTitle: "Nearby",
      listTitle: "Restaurants",
      loadMore: "Load more",
      noResults: "No restaurants found",
    },
    suggestions: {
      guide: "មគ្គុទេសក៍ AI របស់អ្នក។ សួរអ្វីក៏បាន។",
      chips: ["ប្រើយ៉ាងដូចម្តេច?", "អាចធ្វើអ្វីបាន?", "សីលធម៌ការញុំា"],
    },
    restaurant: {
      title: "Select Restaurant",
      subtitle: "Choose a restaurant to explore",
      selectPlaceholder: "Select a restaurant...",
      continue: "Continue",
      noSelection: "Please select a restaurant",
      chatPlaceholder: "សួរអ្វីក៏បាន",
      chatGuide: "មគ្គុទេសក៍ AI សម្រាប់ {name} — សួរអ្វីក៏បានអំពីភោជនីយដ្ឋាននេះ!",
      signatureDish: "អាហារពិសេសនៅទីនេះគឺជាអ្វី?",
      aboutCuisine: "ប្រាប់ខ្ញុំអំពីម្ហូប {cuisine}",
      bestTime: "មានអ្វីណែនាំខ្លះ?",
      dietaryOptions: "ខ្ញុំមានអាហារដែលមិនអាចញ៉ាំបាន",
      undecided: "ខ្ញុំមិនទាន់សម្រេចចិត្តទេ",
      hearty: "អាហារពេញមួយចាន",
      singleDish: "មុខម្ហូបមួយ",
      lightAppetizer: "មុខម្ហូបសម្រន់ / សាឡាត",
      seasonalMenu: "ម៉ឺនុយតាមរដូវ?",
      drinksMenu: "ភេសជ្ជៈ?",
      googleReview: "វាយតម្លៃ",
    },
    nfg: {
      ingredients: "គ្រឿងផ្សំ",
      allergens: "សារធាតុអាឡែកហ្សី",
      restrictions: "ការកំណត់អាហារ",
      flavorProfile: "រសជាតិ",
      calories: "កាឡូរីប៉ាន់ស្មាន",
      taxNote: "ពន្ធ",
      vadBadge: "VAD បានផ្ទៀងផ្ទាត់",
      aiBadge: "AI ប៉ាន់ស្មាន",
      confidence: "ភាពជឿជាក់",
      texture: "វាយនភាព",
      pairing: "គូរជាមួយ",
      howToEat: "របៀបញ៉ាំ",
      story: "រឿងរ៉ាវ",
      servingStyle: "របៀបដាក់ថ្នាំ",
      tasteChart: "រសជាតិ",
      kidFriendly: "សមរម្យសម្រាប់កុមារ",
      notKidFriendly: "ប្រហែលមិនសមរម្យសម្រាប់កុមារ",
      suggestEdit: "ស្នើសុំកែប្រែ",
      abv: "ABV",
      tasteUmami: "អ៊ូម៉ាមី",
      tasteSweetness: "ផ្អែម",
      tasteSourness: "ជូរ",
      tasteSaltiness: "ប្រៃ",
      tasteBitterness: "ល្វីង",
      tasteSpiciness: "ហឹរ",
      tasteRichness: "ចម្រុះ",
      tasteLightness: "ស្រស់",
      tasteVolume: "បរិមាណ",
      tasteLocality: "មូលដ្ឋាន",
    },
    capture: {
      menuAnalysis: "Menu Analysis ({n} items)",
      imageAnalyzedWith: 'Analyzed image of "{q}".',
      imageAnalyzed: "Menu image analyzed.",
      notFound: "Restaurant not found",
      checkUrl: "Please check the URL",
      loading: "Loading...",
      writeReview: "Write a review",
      submitReview: "Write a review",
      reviewPlaceholder: "Food impressions, atmosphere, recommendations...",
      submitting: "Sending...",
      submit: "Submit",
      reviewSubmitted: "Review submitted!",
      comparing: "Comparing",
      compare: "Compare",
      less: "Less",
      moreDetails: "More details",
      relatedMenu: "Related",
      tellMeMore: "Tell me more about #{num} {name}",
      openMap: "Map",
    },
  },
  ne: {
    common: {
      back: "पछाडि",
      reload: "पुनःलोड",
      translate: "अनुवाद",
      openCamera: "क्यामेरा खोल्नुहोस्",
      menu: "मेनु",
      guide: "मार्गदर्शक",
    },
    language: {
      change: "भाषा परिवर्तन",
      select: "भाषा चयन",
      list: "भाषा सूची",
      modalTitle: "भाषा",
      fallback: "भाषा",
    },
    drawer: {
      historyLabel: "इतिहास दराज",
    },
    home: {
      bannerLabel: "ब्यानर",
      bannerAlt: "ब्यानर",
    },
    cta: {
      go: "जानुहोस्",
      continue: "जारी राख्नुहोस्",
    },
    captureHeader: {
      menu: "मेनु",
      scan: "स्क्यान",
    },
    cameraPrompt: {
      openCamera: "क्यामेरा खोल्नुहोस्",
    },
    chat: {
      uploadPreview: "अपलोड पूर्वावलोकन",
      removeAttachment: "संलग्नक हटाउनुहोस्",
      tapToOpen: "इनपुट खोल्न ट्याप गर्नुहोस्",
      camera: "च्याट क्यामेरा",
      gallery: "च्याट ग्यालरी",
      messageInput: "सन्देश इनपुट",
      sendMessage: "सन्देश पठाउनुहोस्",
      send: "पठाउनुहोस्",
      sendDisabled: "पठाउन अक्षम",
      attachmentGuide: "तपाईंको AI मार्गदर्शक। जे पनि सोध्नुहोस्।",
    },
    attachment: {
      cameraPhoto: "क्यामेरा फोटो",
      photoLibrary: "फोटो लाइब्रेरी",
    },
    camera: {
      unavailable: "क्यामेरा उपलब्ध छैन",
      close: "बन्द गर्नुहोस्",
      translate: "अनुवाद",
      openGallery: "ग्यालरी खोल्नुहोस्",
      capturePhoto: "फोटो खिच्नुहोस्",
      flipCamera: "क्यामेरा परिवर्तन",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "पछाडि",
      closeMenu: "मेनु बन्द गर्नुहोस्",
      toggleMenu: "मेनु परिवर्तन",
      nav: {
        stores: "पसलहरू",
        groups: "समूहहरू",
        prompt: "Prompt",
        logout: "लगआउट",
      },
      title: {
        stores: "पसलहरू",
      },
    },
    browser: {
      chromeMock: "ब्राउजर UI मोक",
    },
    history: {
      sectionConversation: "कुराकानी",
      newChat: "नयाँ च्याट",
      empty: "अहिलेसम्म कुनै कुराकानी छैन",
      sectionExplore: "मेनु हेर्नुहोस्",
      favorites: "मनपर्ने",
      popular: "लोकप्रिय",
      sectionStore: "रेस्टुरेन्ट",
      storeInfo: "रेस्टुरेन्ट जानकारी",
      sectionPreferences: "प्राथमिकताहरू",
      preferencesAllergy: "एलर्जी र आहार",
      exploreOther: "अन्य रेस्टुरेन्ट हेर्नुहोस्",
      title: "इतिहास शीर्षक...",
    },
    hero: {
      heading: "खिच्नुहोस् र अनुवाद",
      sub: "सुरु गर्न क्यामेरा ट्याप गर्नुहोस्",
    },
    explore: {
      title: " अन्वेषण",
      subtitle: " सँग अन्वेषण",
      placeholder: "वरपरको बारे सोध्नुहोस्...",
      searchPlaceholder: "Search restaurants",
      allCities: "All",
      nearbyTitle: "Nearby",
      listTitle: "Restaurants",
      loadMore: "Load more",
      noResults: "No restaurants found",
    },
    suggestions: {
      guide: "तपाईंको AI मार्गदर्शक। जे पनि सोध्नुहोस्।",
      chips: ["कसरी प्रयोग गर्ने?", "के गर्न सक्छ?", "भोजन शिष्टाचार"],
    },
    restaurant: {
      title: "Select Restaurant",
      subtitle: "Choose a restaurant to explore",
      selectPlaceholder: "Select a restaurant...",
      continue: "Continue",
      noSelection: "Please select a restaurant",
      chatPlaceholder: "जे पनि सोध्नुहोस्",
      chatGuide: "{name} को लागि AI गाइड — यो रेस्टुरेन्ट बारे जे पनि सोध्नुहोस्!",
      signatureDish: "यहाँको स्पेशल के हो?",
      aboutCuisine: "{cuisine} खानाको बारेमा बताउनुहोस्",
      bestTime: "के खाउँ भन्नुस् न?",
      dietaryOptions: "मैले केही कुरा खान सक्दिनँ",
      undecided: "निर्णय गर्न सकिरहेको छैन",
      hearty: "पेट भरी",
      singleDish: "एउटा डिश",
      lightAppetizer: "स्टार्टर / सलाद",
      seasonalMenu: "सिजनल मेनू?",
      drinksMenu: "पेयपदार्थ?",
      googleReview: "समीक्षा",
    },
    nfg: {
      ingredients: "सामग्री",
      allergens: "एलर्जी",
      restrictions: "आहार प्रतिबन्ध",
      flavorProfile: "स्वाद प्रोफाइल",
      calories: "अनुमानित क्यालोरी",
      taxNote: "कर",
      vadBadge: "VAD प्रमाणित",
      aiBadge: "AI अनुमानित",
      confidence: "विश्वसनीयता",
      texture: "बनावट",
      pairing: "संग मिल्ने",
      howToEat: "कसरी खाने",
      story: "कथा",
      servingStyle: "परोसने शैली",
      tasteChart: "स्वाद",
      kidFriendly: "बच्चाहरूलाई उपयुक्त",
      notKidFriendly: "बच्चाहरूलाई उपयुक्त नहुन सक्छ",
      suggestEdit: "सम्पादन सुझाव",
      abv: "ABV",
      tasteUmami: "उमामी",
      tasteSweetness: "मीठो",
      tasteSourness: "अमिलो",
      tasteSaltiness: "नुनिलो",
      tasteBitterness: "तितो",
      tasteSpiciness: "पिरो",
      tasteRichness: "समृद्ध",
      tasteLightness: "ताजा",
      tasteVolume: "मात्रा",
      tasteLocality: "स्थानीय",
    },
    capture: {
      menuAnalysis: "Menu Analysis ({n} items)",
      imageAnalyzedWith: 'Analyzed image of "{q}".',
      imageAnalyzed: "Menu image analyzed.",
      notFound: "Restaurant not found",
      checkUrl: "Please check the URL",
      loading: "Loading...",
      writeReview: "Write a review",
      submitReview: "Write a review",
      reviewPlaceholder: "Food impressions, atmosphere, recommendations...",
      submitting: "Sending...",
      submit: "Submit",
      reviewSubmitted: "Review submitted!",
      comparing: "Comparing",
      compare: "Compare",
      less: "Less",
      moreDetails: "More details",
      relatedMenu: "Related",
      tellMeMore: "Tell me more about #{num} {name}",
      openMap: "Map",
    },
  },
  mn: {
    common: {
      back: "Буцах",
      reload: "Дахин ачаалах",
      translate: "Орчуулах",
      openCamera: "Камер нээх",
      menu: "Цэс",
      guide: "Заавар",
    },
    language: {
      change: "Хэл солих",
      select: "Хэл сонгох",
      list: "Хэлний жагсаалт",
      modalTitle: "Хэл",
      fallback: "Хэл",
    },
    drawer: {
      historyLabel: "Түүхийн шургуулга",
    },
    home: {
      bannerLabel: "Баннер",
      bannerAlt: "Баннер",
    },
    cta: {
      go: "Явах",
      continue: "Үргэлжлүүлэх",
    },
    captureHeader: {
      menu: "Цэс",
      scan: "Скан",
    },
    cameraPrompt: {
      openCamera: "Камер нээх",
    },
    chat: {
      uploadPreview: "Байршуулалтын урьдчилсан харах",
      removeAttachment: "Хавсралт устгах",
      tapToOpen: "Оруулахыг нээхээр товшино уу",
      camera: "Чатын камер",
      gallery: "Чатын галерей",
      messageInput: "Мессеж оруулах",
      sendMessage: "Мессеж илгээх",
      send: "Илгээх",
      sendDisabled: "Илгээх идэвхгүй",
      attachmentGuide: "Таны AI гарын авлага. Юу ч асуугаарай.",
    },
    attachment: {
      cameraPhoto: "Камерын зураг",
      photoLibrary: "Зургийн сан",
    },
    camera: {
      unavailable: "Камер ашиглах боломжгүй",
      close: "Хаах",
      translate: "Орчуулах",
      openGallery: "Галерей нээх",
      capturePhoto: "Зураг авах",
      flipCamera: "Камер солих",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "Буцах",
      closeMenu: "Цэс хаах",
      toggleMenu: "Цэс солих",
      nav: {
        stores: "Дэлгүүрүүд",
        groups: "Бүлгүүд",
        prompt: "Prompt",
        logout: "Гарах",
      },
      title: {
        stores: "Дэлгүүрүүд",
      },
    },
    browser: {
      chromeMock: "Хөтөч UI загвар",
    },
    history: {
      sectionConversation: "Яриа",
      newChat: "Шинэ чат",
      empty: "Одоогоор яриа алга",
      sectionExplore: "Цэс үзэх",
      favorites: "Дуртай",
      popular: "Алдартай",
      sectionStore: "Ресторан",
      storeInfo: "Рестораны мэдээлэл",
      sectionPreferences: "Тохиргоо",
      preferencesAllergy: "Харшил ба хооллолт",
      exploreOther: "Бусад ресторан үзэх",
      title: "Түүхийн гарчиг...",
    },
    hero: {
      heading: "Зураг аваад орчуул",
      sub: "Эхлэхийн тулд камерыг товшино уу",
    },
    explore: {
      title: "-г судлах",
      subtitle: "-тай судлах",
      placeholder: "Ойр орчмын талаар асуугаарай...",
      searchPlaceholder: "Search restaurants",
      allCities: "All",
      nearbyTitle: "Nearby",
      listTitle: "Restaurants",
      loadMore: "Load more",
      noResults: "No restaurants found",
    },
    suggestions: {
      guide: "Таны AI гарын авлага. Юу ч асуугаарай.",
      chips: ["Яаж ашиглах вэ?", "Юу хийж чадах вэ?", "Хоолны ёс зүй"],
    },
    restaurant: {
      title: "Select Restaurant",
      subtitle: "Choose a restaurant to explore",
      selectPlaceholder: "Select a restaurant...",
      continue: "Continue",
      noSelection: "Please select a restaurant",
      chatPlaceholder: "Юу ч асуугаарай",
      chatGuide: "{name}-ийн AI гарын авлага — Энэ ресторан дээр юу ч асуугаарай!",
      signatureDish: "Энд юу онцгой вэ?",
      aboutCuisine: "{cuisine} хоолны тухай хэлнэ үү",
      bestTime: "Юу санал болгох вэ?",
      dietaryOptions: "Надад иддэггүй зүйл бий",
      undecided: "Шийдэж чадахгүй байна",
      hearty: "Цадтал",
      singleDish: "Нэг таваг",
      lightAppetizer: "Зууш / Салат",
      seasonalMenu: "Улирлын цэс?",
      drinksMenu: "Ундаа?",
      googleReview: "Сэтгэгдэл",
    },
    nfg: {
      ingredients: "Орц",
      allergens: "Харшил",
      restrictions: "Хоолны хязгаарлалт",
      flavorProfile: "Амтын профайл",
      calories: "Тооцоолсон калори",
      taxNote: "Татвар",
      vadBadge: "VAD Баталгаажсан",
      aiBadge: "AI Тооцоолсон",
      confidence: "Итгэлцэл",
      texture: "Бүтэц",
      pairing: "Хослох",
      howToEat: "Хэрхэн идэх",
      story: "Түүх",
      servingStyle: "Үйлчилгээ",
      tasteChart: "Амт",
      kidFriendly: "Хүүхдэд тохиромжтой",
      notKidFriendly: "Хүүхдэд тохиромжгүй байж болно",
      suggestEdit: "Засвар санал болгох",
      abv: "ABV",
      tasteUmami: "Умами",
      tasteSweetness: "Чихэрлэг",
      tasteSourness: "Гашуун",
      tasteSaltiness: "Давстай",
      tasteBitterness: "Гашуун",
      tasteSpiciness: "Халуун",
      tasteRichness: "Баялаг",
      tasteLightness: "Сэрүүн",
      tasteVolume: "Хэмжээ",
      tasteLocality: "Нутгийн",
    },
    capture: {
      menuAnalysis: "Menu Analysis ({n} items)",
      imageAnalyzedWith: 'Analyzed image of "{q}".',
      imageAnalyzed: "Menu image analyzed.",
      notFound: "Restaurant not found",
      checkUrl: "Please check the URL",
      loading: "Loading...",
      writeReview: "Write a review",
      submitReview: "Write a review",
      reviewPlaceholder: "Food impressions, atmosphere, recommendations...",
      submitting: "Sending...",
      submit: "Submit",
      reviewSubmitted: "Review submitted!",
      comparing: "Comparing",
      compare: "Compare",
      less: "Less",
      moreDetails: "More details",
      relatedMenu: "Related",
      tellMeMore: "Tell me more about #{num} {name}",
      openMap: "Map",
    },
  },
  fa: {
    common: {
      back: "بازگشت",
      reload: "تازه‌سازی",
      translate: "ترجمه",
      openCamera: "باز کردن دوربین",
      menu: "منو",
      guide: "راهنما",
    },
    language: {
      change: "تغییر زبان",
      select: "انتخاب زبان",
      list: "فهرست زبان‌ها",
      modalTitle: "زبان",
      fallback: "زبان",
    },
    drawer: {
      historyLabel: "کشوی تاریخچه",
    },
    home: {
      bannerLabel: "بنر",
      bannerAlt: "بنر",
    },
    cta: {
      go: "برو",
      continue: "ادامه",
    },
    captureHeader: {
      menu: "منو",
      scan: "اسکن",
    },
    cameraPrompt: {
      openCamera: "باز کردن دوربین",
    },
    chat: {
      uploadPreview: "پیش‌نمایش بارگذاری",
      removeAttachment: "حذف پیوست",
      tapToOpen: "برای باز کردن ورودی ضربه بزنید",
      camera: "دوربین گفتگو",
      gallery: "گالری گفتگو",
      messageInput: "ورودی پیام",
      sendMessage: "ارسال پیام",
      send: "ارسال",
      sendDisabled: "ارسال غیرفعال",
      attachmentGuide: "راهنمای هوش مصنوعی شما. هر چیزی بپرسید.",
    },
    attachment: {
      cameraPhoto: "عکس دوربین",
      photoLibrary: "کتابخانه عکس",
    },
    camera: {
      unavailable: "دوربین در دسترس نیست",
      close: "بستن",
      translate: "ترجمه",
      openGallery: "باز کردن گالری",
      capturePhoto: "گرفتن عکس",
      flipCamera: "تعویض دوربین",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "بازگشت",
      closeMenu: "بستن منو",
      toggleMenu: "تعویض منو",
      nav: {
        stores: "فروشگاه‌ها",
        groups: "گروه‌ها",
        prompt: "Prompt",
        logout: "خروج",
      },
      title: {
        stores: "فروشگاه‌ها",
      },
    },
    browser: {
      chromeMock: "ماک رابط مرورگر",
    },
    history: {
      sectionConversation: "گفتگوها",
      newChat: "گفتگوی جدید",
      empty: "هنوز گفتگویی نیست",
      sectionExplore: "کاوش منو",
      favorites: "علاقه‌مندی‌ها",
      popular: "محبوب",
      sectionStore: "رستوران",
      storeInfo: "اطلاعات رستوران",
      sectionPreferences: "تنظیمات",
      preferencesAllergy: "حساسیت‌ها و رژیم غذایی",
      exploreOther: "دیدن رستوران‌های دیگر",
      title: "عنوان تاریخچه...",
    },
    hero: {
      heading: "عکس بگیر و ترجمه کن",
      sub: "برای شروع روی دوربین ضربه بزنید",
    },
    explore: {
      title: "کاوش ",
      subtitle: "با  کاوش کنید",
      placeholder: "درباره اطراف بپرسید...",
      searchPlaceholder: "Search restaurants",
      allCities: "All",
      nearbyTitle: "Nearby",
      listTitle: "Restaurants",
      loadMore: "Load more",
      noResults: "No restaurants found",
    },
    suggestions: {
      guide: "راهنمای هوش مصنوعی شما. هر چیزی بپرسید.",
      chips: ["چطور استفاده کنم؟", "چه کارهایی می‌تواند انجام دهد؟", "آداب غذاخوری"],
    },
    restaurant: {
      title: "Select Restaurant",
      subtitle: "Choose a restaurant to explore",
      selectPlaceholder: "Select a restaurant...",
      continue: "Continue",
      noSelection: "Please select a restaurant",
      chatPlaceholder: "هر چیزی بپرسید",
      chatGuide: "راهنمای هوش مصنوعی برای {name} — هر چیزی درباره این رستوران بپرسید!",
      signatureDish: "غذای مخصوص اینجا چیه؟",
      aboutCuisine: "درباره غذاهای {cuisine} بگویید",
      bestTime: "چی پیشنهاد میدی؟",
      dietaryOptions: "یه سری چیزا رو نمیتونم بخورم",
      undecided: "دودلم",
      hearty: "غذای سنگین",
      singleDish: "یه بشقاب",
      lightAppetizer: "پیش‌غذا / سالاد",
      seasonalMenu: "منوی فصلی؟",
      drinksMenu: "نوشیدنی؟",
      googleReview: "نظر",
    },
    nfg: {
      ingredients: "مواد تشکیل‌دهنده",
      allergens: "آلرژن‌ها",
      restrictions: "محدودیت‌های غذایی",
      flavorProfile: "پروفایل طعم",
      calories: "کالری تقریبی",
      taxNote: "مالیات",
      vadBadge: "تأیید شده VAD",
      aiBadge: "تخمین AI",
      confidence: "اطمینان",
      texture: "بافت",
      pairing: "همراه با",
      howToEat: "نحوه خوردن",
      story: "داستان",
      servingStyle: "نحوه سرو",
      tasteChart: "طعم",
      kidFriendly: "مناسب کودکان",
      notKidFriendly: "ممکن است برای کودکان مناسب نباشد",
      suggestEdit: "پیشنهاد ویرایش",
      abv: "ABV",
      tasteUmami: "اومامی",
      tasteSweetness: "شیرین",
      tasteSourness: "ترش",
      tasteSaltiness: "شور",
      tasteBitterness: "تلخ",
      tasteSpiciness: "تند",
      tasteRichness: "غنی",
      tasteLightness: "تازه",
      tasteVolume: "حجم",
      tasteLocality: "محلی",
    },
    capture: {
      menuAnalysis: "Menu Analysis ({n} items)",
      imageAnalyzedWith: 'Analyzed image of "{q}".',
      imageAnalyzed: "Menu image analyzed.",
      notFound: "Restaurant not found",
      checkUrl: "Please check the URL",
      loading: "Loading...",
      writeReview: "Write a review",
      submitReview: "Write a review",
      reviewPlaceholder: "Food impressions, atmosphere, recommendations...",
      submitting: "Sending...",
      submit: "Submit",
      reviewSubmitted: "Review submitted!",
      comparing: "Comparing",
      compare: "Compare",
      less: "Less",
      moreDetails: "More details",
      relatedMenu: "Related",
      tellMeMore: "Tell me more about #{num} {name}",
      openMap: "Map",
    },
  },
  uk: {
    common: {
      back: "Назад",
      reload: "Оновити",
      translate: "Перекласти",
      openCamera: "Відкрити камеру",
      menu: "Меню",
      guide: "Гід",
    },
    language: {
      change: "Змінити мову",
      select: "Вибрати мову",
      list: "Список мов",
      modalTitle: "Мова",
      fallback: "Мова",
    },
    drawer: {
      historyLabel: "Панель історії",
    },
    home: {
      bannerLabel: "Банер",
      bannerAlt: "Банер",
    },
    cta: {
      go: "Перейти",
      continue: "Продовжити",
    },
    captureHeader: {
      menu: "Меню",
      scan: "Сканувати",
    },
    cameraPrompt: {
      openCamera: "Відкрити камеру",
    },
    chat: {
      uploadPreview: "Попередній перегляд завантаження",
      removeAttachment: "Видалити вкладення",
      tapToOpen: "Натисніть, щоб відкрити введення",
      camera: "Камера чату",
      gallery: "Галерея чату",
      messageInput: "Поле повідомлення",
      sendMessage: "Надіслати повідомлення",
      send: "Надіслати",
      sendDisabled: "Надсилання вимкнено",
      attachmentGuide: "Ваш AI-гід. Питайте будь-що.",
    },
    attachment: {
      cameraPhoto: "Фото з камери",
      photoLibrary: "Фотобібліотека",
    },
    camera: {
      unavailable: "Камера недоступна",
      close: "Закрити",
      translate: "Перекласти",
      openGallery: "Відкрити галерею",
      capturePhoto: "Зробити фото",
      flipCamera: "Перемкнути камеру",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "Назад",
      closeMenu: "Закрити меню",
      toggleMenu: "Перемкнути меню",
      nav: {
        stores: "Магазини",
        groups: "Групи",
        prompt: "Prompt",
        logout: "Вийти",
      },
      title: {
        stores: "Магазини",
      },
    },
    browser: {
      chromeMock: "Макет інтерфейсу браузера",
    },
    history: {
      sectionConversation: "Розмови",
      newChat: "Новий чат",
      empty: "Розмов поки немає",
      sectionExplore: "Меню",
      favorites: "Обране",
      popular: "Популярне",
      sectionStore: "Заклад",
      storeInfo: "Про заклад",
      sectionPreferences: "Налаштування",
      preferencesAllergy: "Алергії та харчування",
      exploreOther: "Інші заклади",
      title: "Заголовок історії...",
    },
    hero: {
      heading: "Знімай і перекладай",
      sub: "Натисніть камеру, щоб почати",
    },
    explore: {
      title: "Досліджуйте ",
      subtitle: "Досліджуйте з ",
      placeholder: "Питайте про місцевість...",
      searchPlaceholder: "Search restaurants",
      allCities: "All",
      nearbyTitle: "Nearby",
      listTitle: "Restaurants",
      loadMore: "Load more",
      noResults: "No restaurants found",
    },
    suggestions: {
      guide: "Ваш AI-гід. Питайте будь-що.",
      chips: ["Як користуватися?", "Що вміє?", "Етикет за столом"],
    },
    restaurant: {
      title: "Select Restaurant",
      subtitle: "Choose a restaurant to explore",
      selectPlaceholder: "Select a restaurant...",
      continue: "Continue",
      noSelection: "Please select a restaurant",
      chatPlaceholder: "Питайте будь-що",
      chatGuide: "AI-гід для {name} — Питайте будь-що про цей ресторан!",
      signatureDish: "Що тут фірмове?",
      aboutCuisine: "Розкажіть про кухню {cuisine}",
      bestTime: "Що порадите?",
      dietaryOptions: "Я дещо не можу їсти",
      undecided: "Не можу обрати",
      hearty: "Ситна страва",
      singleDish: "Одна страва",
      lightAppetizer: "Закуска / Салат",
      seasonalMenu: "Сезонне меню?",
      drinksMenu: "Напої?",
      googleReview: "Відгук",
    },
    nfg: {
      ingredients: "Інгредієнти",
      allergens: "Алергени",
      restrictions: "Дієтичні обмеження",
      flavorProfile: "Смаковий профіль",
      calories: "Орієнт. калорії",
      taxNote: "Податок",
      vadBadge: "VAD Перевірено",
      aiBadge: "Оцінка AI",
      confidence: "Достовірність",
      texture: "Текстура",
      pairing: "Пасує до",
      howToEat: "Як їсти",
      story: "Історія",
      servingStyle: "Подача",
      tasteChart: "Смак",
      kidFriendly: "Підходить дітям",
      notKidFriendly: "Може не підходити дітям",
      suggestEdit: "Запропонувати зміну",
      abv: "ABV",
      tasteUmami: "Умамі",
      tasteSweetness: "Солодкий",
      tasteSourness: "Кислий",
      tasteSaltiness: "Солоний",
      tasteBitterness: "Гіркий",
      tasteSpiciness: "Гострий",
      tasteRichness: "Насичений",
      tasteLightness: "Свіжий",
      tasteVolume: "Об'єм",
      tasteLocality: "Місцевий",
    },
    capture: {
      menuAnalysis: "Menu Analysis ({n} items)",
      imageAnalyzedWith: 'Analyzed image of "{q}".',
      imageAnalyzed: "Menu image analyzed.",
      notFound: "Restaurant not found",
      checkUrl: "Please check the URL",
      loading: "Loading...",
      writeReview: "Write a review",
      submitReview: "Write a review",
      reviewPlaceholder: "Food impressions, atmosphere, recommendations...",
      submitting: "Sending...",
      submit: "Submit",
      reviewSubmitted: "Review submitted!",
      comparing: "Comparing",
      compare: "Compare",
      less: "Less",
      moreDetails: "More details",
      relatedMenu: "Related",
      tellMeMore: "Tell me more about #{num} {name}",
      openMap: "Map",
    },
  },
  pl: {
    common: {
      back: "Wstecz",
      reload: "Odśwież",
      translate: "Tłumacz",
      openCamera: "Otwórz aparat",
      menu: "Menu",
      guide: "Przewodnik",
    },
    language: {
      change: "Zmień język",
      select: "Wybierz język",
      list: "Lista języków",
      modalTitle: "Język",
      fallback: "Język",
    },
    drawer: {
      historyLabel: "Szuflada historii",
    },
    home: {
      bannerLabel: "Baner",
      bannerAlt: "Baner",
    },
    cta: {
      go: "Idź",
      continue: "Kontynuuj",
    },
    captureHeader: {
      menu: "Menu",
      scan: "Skanuj",
    },
    cameraPrompt: {
      openCamera: "Otwórz aparat",
    },
    chat: {
      uploadPreview: "Podgląd przesyłania",
      removeAttachment: "Usuń załącznik",
      tapToOpen: "Dotknij, aby otworzyć wpisywanie",
      camera: "Kamera czatu",
      gallery: "Galeria czatu",
      messageInput: "Pole wiadomości",
      sendMessage: "Wyślij wiadomość",
      send: "Wyślij",
      sendDisabled: "Wysyłanie wyłączone",
      attachmentGuide: "Twój przewodnik AI. Pytaj o wszystko.",
    },
    attachment: {
      cameraPhoto: "Zdjęcie z aparatu",
      photoLibrary: "Biblioteka zdjęć",
    },
    camera: {
      unavailable: "Aparat niedostępny",
      close: "Zamknij",
      translate: "Tłumacz",
      openGallery: "Otwórz galerię",
      capturePhoto: "Zrób zdjęcie",
      flipCamera: "Przełącz aparat",
    },
    admin: {
      brand: "OMISEAI Admin",
      back: "Wstecz",
      closeMenu: "Zamknij menu",
      toggleMenu: "Przełącz menu",
      nav: {
        stores: "Sklepy",
        groups: "Grupy",
        prompt: "Prompt",
        logout: "Wyloguj",
      },
      title: {
        stores: "Sklepy",
      },
    },
    browser: {
      chromeMock: "Makieta przeglądarki",
    },
    history: {
      sectionConversation: "Rozmowy",
      newChat: "Nowy czat",
      empty: "Brak rozmów",
      sectionExplore: "Przeglądaj menu",
      favorites: "Ulubione",
      popular: "Popularne",
      sectionStore: "Restauracja",
      storeInfo: "Informacje o restauracji",
      sectionPreferences: "Preferencje",
      preferencesAllergy: "Alergie i dieta",
      exploreOther: "Zobacz inne restauracje",
      title: "Tytuł historii...",
    },
    hero: {
      heading: "Zrób zdjęcie i tłumacz",
      sub: "Dotknij aparatu, aby zacząć",
    },
    explore: {
      title: "Odkrywaj ",
      subtitle: "Odkrywaj z ",
      placeholder: "Zapytaj o okolicę...",
      searchPlaceholder: "Search restaurants",
      allCities: "All",
      nearbyTitle: "Nearby",
      listTitle: "Restaurants",
      loadMore: "Load more",
      noResults: "No restaurants found",
    },
    suggestions: {
      guide: "Twój przewodnik AI. Pytaj o wszystko.",
      chips: ["Jak używać?", "Co potrafi?", "Etykieta przy stole"],
    },
    restaurant: {
      title: "Select Restaurant",
      subtitle: "Choose a restaurant to explore",
      selectPlaceholder: "Select a restaurant...",
      continue: "Continue",
      noSelection: "Please select a restaurant",
      chatPlaceholder: "Pytaj o cokolwiek",
      chatGuide: "Przewodnik AI dla {name} — Pytaj o wszystko dotyczące tej restauracji!",
      signatureDish: "Co jest tu specjalnością?",
      aboutCuisine: "Opowiedz mi o kuchni {cuisine}",
      bestTime: "Co polecacie?",
      dietaryOptions: "Są rzeczy, których nie mogę jeść",
      undecided: "Nie mogę się zdecydować",
      hearty: "Coś sytego",
      singleDish: "Jedno danie",
      lightAppetizer: "Przystawka / Sałatka",
      seasonalMenu: "Menu sezonowe?",
      drinksMenu: "Napoje?",
      googleReview: "Opinia",
    },
    nfg: {
      ingredients: "Sk\u0142adniki",
      allergens: "Alergeny",
      restrictions: "Ograniczenia dietetyczne",
      flavorProfile: "Profil smakowy",
      calories: "Szac. kalorie",
      taxNote: "Podatek",
      vadBadge: "Zweryfikowano VAD",
      aiBadge: "Oszacowano przez AI",
      confidence: "Pewno\u015B\u0107",
      texture: "Tekstura",
      pairing: "Pasuje do",
      howToEat: "Jak je\u015B\u0107",
      story: "Historia",
      servingStyle: "Podanie",
      tasteChart: "Smak",
      kidFriendly: "Dla dzieci",
      notKidFriendly: "Mo\u017Ce nie odpowiada\u0107 dzieciom",
      suggestEdit: "Zaproponuj zmianę",
      abv: "ABV",
      tasteUmami: "Umami",
      tasteSweetness: "Słodki",
      tasteSourness: "Kwaśny",
      tasteSaltiness: "Słony",
      tasteBitterness: "Gorzki",
      tasteSpiciness: "Ostry",
      tasteRichness: "Bogaty",
      tasteLightness: "Świeży",
      tasteVolume: "Objętość",
      tasteLocality: "Lokalny",
    },
    capture: {
      menuAnalysis: "Menu Analysis ({n} items)",
      imageAnalyzedWith: 'Analyzed image of "{q}".',
      imageAnalyzed: "Menu image analyzed.",
      notFound: "Restaurant not found",
      checkUrl: "Please check the URL",
      loading: "Loading...",
      writeReview: "Write a review",
      submitReview: "Write a review",
      reviewPlaceholder: "Food impressions, atmosphere, recommendations...",
      submitting: "Sending...",
      submit: "Submit",
      reviewSubmitted: "Review submitted!",
      comparing: "Comparing",
      compare: "Compare",
      less: "Less",
      moreDetails: "More details",
      relatedMenu: "Related",
      tellMeMore: "Tell me more about #{num} {name}",
      openMap: "Map",
    },
  },
} as const;

export type UiCopy = DeepStringify<(typeof uiCopy)["en"]>;

const normalizeLanguage = (code?: string) => {
  if (!code) return "en";
  if (code in uiCopy) return code as keyof typeof uiCopy;
  const lower = code.toLowerCase();
  const base = lower.split("-")[0];
  if (base in uiCopy) return base as keyof typeof uiCopy;
  return "en";
};

export const getUiCopy = (language?: string): UiCopy => {
  const normalized = normalizeLanguage(language);
  const copy = uiCopy[normalized];
  // Provide fallback for missing sections
  const result = { ...copy } as Record<string, unknown>;
  if (!('qrScanner' in copy)) {
    result.qrScanner = uiCopy.en.qrScanner;
  }
  if (!('nfg' in copy)) {
    result.nfg = uiCopy.en.nfg;
  }
  if (!('feedback' in copy)) {
    result.feedback = uiCopy.en.feedback;
  }
  if (!('capture' in copy)) {
    result.capture = uiCopy.en.capture;
  }
  if (!('quickExplain' in copy)) {
    result.quickExplain = uiCopy.en.quickExplain;
  }
  if (!('webLanding' in copy)) {
    result.webLanding = uiCopy.en.webLanding;
  }
  return result as UiCopy;
};

export const getLanguageLabel = (code?: string) => {
  if (!code) return undefined;
  return languageLabelMap.get(code) ?? code;
};
