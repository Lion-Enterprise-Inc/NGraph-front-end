...
const adminCopy: Record<AdminLang, AdminCopy> = {
  ja: {
    ...
    menuList: {
      ...
      confirmBulkApprove: (n) => `${n}件のメニューをまとめて提供中にしますか？`,
      ...
      tableFilterVerified: (n) => `提供中 (${n})`,
      tableFilterWarning: (n) => `アーカイブ (${n})`,
      ...
      tableReload: '再読み込み',
      tableBulkApproveCount: (n) => `まとめて提供中にする (${n}件)`,
      tableColNo: 'No.',
      tableColDetail: 'メニュー詳細',
      tableEmpty: 'メニューがありません。「手動で新規追加」ボタンからメニューを追加してください。',
      tableNoIngredients: '原材料なし',
      tableStatusPublic: '提供中',
      tableStatusPrivate: 'アーカイブ',
      tableStatusVerified: '提供中',
      tableStatusPending: 'アーカイブ',
    ...
    prompts: {
      ...
      intro: '基本ルール（メニュー案内・多言語対応など）は自動で効いています。ここではAIの接客トーンと、「うちはこうしてほしい」というお店独自の指示を自由に書けます。下の例文から選んで足すこともできます。',
      aiTone: 'AIトーン',
      additionalInstructions: 'お店からAIへの指示（自由記述）',
      promptPlaceholder: 'お店のことをAIに自由に教えてください。\n\n例）\n・アレルギーを聞かれたら、必ず該当メニューを確認してから答えて\n・おすすめは当店名物を理由つきで一押しして\n・予約や営業時間はお電話でのご予約をご案内して\n・提供していないメニューは無理に勧めず正直に伝えて',
      charCount: (n) => `文字数: ${n}`,
      advanced: '詳細設定（任意）',
...
5. そのレストランの話題のみに応答を限定`,
  ...
  en: {
    ...
    menuList: {
      ...
      confirmBulkApprove: (n) => `Activate all ${n} menus?`,
      ...
      tableFilterVerified: (n) => `Active (${n})`,
      tableFilterWarning: (n) => `Archived (${n})`,
      ...
      tableReload: 'Reload',
      tableBulkApproveCount: (n) => `Activate all (${n})`,
      tableColNo: '#',
      tableColDetail: 'Menu detail',
      tableEmpty: 'No menus yet. Click "Add manually" to add one.',
      tableNoIngredients: 'No ingredients',
      tableStatusPublic: 'Active',
      tableStatusPrivate: 'Archived',
      tableStatusVerified: 'Active',
      tableStatusPending: 'Archived',
    ...
    storeKnowledge: {
      title: 'Store Interview',
      notFound: 'Restaurant not found',
      headerTitle: 'Store Interview (v2.1)',
    ...
    prompts: {
      ...
      intro: 'Base rules (menu guidance, multilingual support, etc.) run automatically. Here you can freely set the AI\'s tone and your own store instructions ("we want it handled this way"). You can also insert examples below.',
      aiTone: 'AI tone',
      additionalInstructions: 'Your instructions to the AI (free text)',
      promptPlaceholder: 'Tell the AI about your store, freely.\n\ne.g.\n- When asked about allergies, always check the relevant menu before answering\n- Recommend our signature dish as the top pick, with reasons\n- For reservations or hours, guide guests to call us\n- Don\'t push items that are not served; be honest',
      charCount: (n) => `Chars: ${n}`,
...
5. Limit replies to topics about this restaurant only`,
    ...
    nav: {
      ...
      storeKnowledge: 'Store Interview',
