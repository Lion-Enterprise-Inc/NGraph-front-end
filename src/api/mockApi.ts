...
export async function mockScanResponse({
  ...
  // If user sent text (no photo), show greeting
  if (message && !attachmentLabel) {
    return {
      title: '😊こんにちは！',
      intro: '料理の名前やメニュー表の写真があれば、その料理の内容やアレルゲン情報などを詳しくお伝えできます。',
      body: ['どうぞ、お気軽に写真をアップロードするか、料理名を教えてください！'],
    }
  }
  
  // If user sent photo (with or without text), show menu explanation
  return {
    title: '🍜 メニュー解説',
    intro: '塩梅らーめん（しおうめらーめん） - 920円',
    body: [
