...
export interface QuickExplainItem {
  ...
  drink_meta?: {
    kind?: 'sake' | 'wine' | 'beer' | 'cocktail' | 'spirit' | 'whisky' | 'soft_drink' | string;
    brand?: string;
    brewery?: string;
    ...
    rice_variety?: string;
    rice_origin?: string;
    polishing_ratio_pct?: number;
    sake_meter_value?: string | number;
    abv_pct?: number;
...
export interface OwnerQuestion {
  ...
  question: string;
  options: string[];
  // kitchen = 厨房共通質問(回答が該当メニュー全部に波及) / store = 店舗プロフィール質問
  // (回答が店の記録フィールドに直接入る)。どちらも「お店全体」表示でundo/ひとこと対象外
